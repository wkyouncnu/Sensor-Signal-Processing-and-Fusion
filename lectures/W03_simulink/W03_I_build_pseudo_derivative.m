function W03_I_build_pseudo_derivative()
%W03_I_BUILD_PSEUDO_DERIVATIVE  유사미분 모델을 만든다 — 실제 PID 가 결코
%                               직접 미분하지 않는 이유.
%                               Generate W03_I_pseudo_derivative.slx: why a
...
%       3주차의 서지 축은 미분 동작이 도움이 되지 않는 축이다. 그래서 절 A 부터
%       G 까지의 어디에서도 미분 필터가 **무엇을 위한 것인지**는 드러나지 않는다.
%       이 모델은 미분이 실제로 필요한 플랜트를 가져와, 필터가 무엇을 사고 무엇을
%       파는지 보인다.
%
%       The surge axis of Week 3 is one on which derivative action does not
...
%
%   Section 3-4 writes the derivative term as
%
...
%
%   Regenerating is safe: any existing W03_I_pseudo_derivative.slx is overwritten.

m    = 'W03_I_pseudo_derivative';
...
%  given in 3-4: a step in r would otherwise be differentiated into an impulse.
...
'on the surge axis of 3-4 where it only added mass.'
''
...
add_block('simulink/Sinks/To Workspace', [q '/W03pd'], ...
          'VariableName','W03pd', 'SaveFormat','Structure With Time', ...
...
row_feed(q, 'log', QIN);
add_line(q, 'log/1','W03pd/1','autorouting','on');
...
'WEEK 3, SECTION I  -  THE PSEUDO-DERIVATIVE'
''
...
''
'Edit pd_Kp, pd_Kd, pd_N1, pd_N2 and pd_noise in W03_0_setup.m.'}, newline));

set_param(m, 'StopFcn', 'W03_pd_plot;');
