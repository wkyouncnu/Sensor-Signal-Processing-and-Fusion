%% W03 · 절 E — 적분 동작, 그리고 미분항이 실제로 놓이는 자리
%  W03 · Section E — integral action, and where the derivative term goes
%
...
%      Part 2 의 절 E 이며 §3-3 과 §3-4 를 함께 받는다. §3-3 은 감쇠비와 고유
%      진동수로 PI 게인을 정하는 법을 유도했고, §3-4 는 미분항이 이 축에서
%      감쇠가 아니라 질량으로 작용함을 보였다. 두 주장을 차례로 확인한다.
%
%      This is section E of Part 2 and it answers both §3-3, which designed
%      the PI gains from a damping ratio and a natural frequency, and §3-4,
...
%      img/W03_result_PI.png   PI 설계와, 아무도 배치하지 않은 영점
%                              the PI design, and the zero nobody placed
%      img/W03_result_D.png    미분 동작이 오히려 나쁘게 만드는 모습
...
%  =====================================================================
RI = run_sim('W03_surge_control', V, ...
...
fprintf('\n  W03 section E — PI for zeta = %.2f, wn = %.2f rad/s\n\n', V.zeta_d, V.wn_d);
...
title({'what the zero costs', ...
       sprintf('identical poles; overshoot %.1f %% against %.1f %%', ...
               S.Overshoot, Mp_pole)});
sgtitle('W03 E — integral action, and the zero nobody placed', 'FontWeight','bold');
exportgraphics(f, fullfile(here,'img','W03_result_PI.png'), 'Resolution', 150);
...
for i = 1:numel(KDS)
    ...
    RD{i}  = run_sim('W03_surge_control', V, ...
...
fprintf(['\n    DERIVATIVE ACTION MAKES THIS LOOP WORSE, AND NOT THROUGH A SIGN\n' ...
         ...
         '\n      This is a property of the AXIS and not of PID. Week 4 controls\n' ...
...
f = lab_fig('W03 E  derivative', 950, 400);
subplot(1,2,1); hold on;
yline(V.u_d1, 'k:', 'DisplayName','setpoint');
...
sgtitle('W03 E — on a velocity loop the derivative term is a mass', 'FontWeight','bold');
exportgraphics(f, fullfile(here,'img','W03_result_D.png'), 'Resolution', 150);

fprintf('\n  figures -> img/W03_result_PI.png, img/W03_result_D.png\n\n');
