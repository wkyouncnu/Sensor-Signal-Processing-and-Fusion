%% W03 · 절 I — 유사미분
%  W03 · Section I — the pseudo-derivative
%
...
%      Part 2 의 절 I 이며 §3-4 의 "유사미분" 절을 받는다. 이번 주의 서지 축은
...
%      of §3-4. The surge axis of this week is one on which derivative action
...
%      표 셋, 블록도, 그리고 img/W03_result_pd.png
%      Three tables, the block diagram, and img/W03_result_pd.png

varargin = {};   % kept so the override loop below still works unchanged
%W03_PD_RUN  Run W03_I_pseudo_derivative.slx and report what the derivative filter buys.
%
%   W03_I_pseudo_derivative                      the four rows of section I
%   W03_I_pseudo_derivative('pd_noise', 0)       override any variable
%   out = W03_I_pseudo_derivative(...)           out.t, out.y
%
%   THE QUESTION
%
...
%
%   Produces
%     img/W03_I_pseudo_derivative.png              the block diagram
%     img/W03_result_pd.png           the four rows: output, actuator, D term
%     (the N sweep prints a table; its figure was withdrawn — see below)

here = fileparts(mfilename('fullpath'));
root = fileparts(fileparts(here));
addpath(fullfile(root,'_tools'), here);
if ~isfile(fullfile(here,'W03_I_pseudo_derivative.slx')), W03_I_build_pseudo_derivative(); end
...
load_system('W03_I_pseudo_derivative');
print('-sW03_I_pseudo_derivative', '-dpng', '-r150', img('W03_I_pseudo_derivative.png'));
close_system('W03_I_pseudo_derivative', 0);

f = W03_pd_plot({R}, LBL, V, ...
    'W03 section I — one derivative, taken four ways, against the same noise');
exportgraphics(f, img('W03_result_pd.png'), 'Resolution', 150);

%  WITHDRAWN 2026-09-08, at the lecturer's request: the two-panel N sweep
%  figure that used to be saved here as img/W03_result_pd_N.png.
%
...
%  and measurement provenance is not what §10-8 asks to cut.

fprintf('\n  figure written to %s\n\n', img('W03_result_pd.png'));
%  (the old function-closing end was here; this file is a script now)

% =========================================================================
function o = one(V, varargin)
for i = 1:2:numel(varargin), V.(varargin{i}) = varargin{i+1}; end
in = Simulink.SimulationInput('W03_I_pseudo_derivative');
fn = fieldnames(V);
for i = 1:numel(fn), in = in.setVariable(fn{i}, V.(fn{i})); end
evalc('r = sim(in);');
y = squeeze(r.W03pd.signals.values);
