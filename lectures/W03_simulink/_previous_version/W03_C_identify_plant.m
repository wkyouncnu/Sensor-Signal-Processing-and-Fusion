%% W03 · 절 C — 플랜트를 플랜트 자신으로부터 동정한다
%  W03 · Section C — identify the plant from the plant
%
...
%      Part 2 의 절 C 이며 §3-1 과 짝을 이룬다. §3-1 은 6자유도 모델에서 출발해
%      전진축만 남기고 그것을 1차계 두 개의 수 K_u 와 T_u 로 줄였다. 이 절은
%      그 두 수를 모델에서 직접 재어 유도가 옳았는지 확인한다.
%
%      This is section C of Part 2, the counterpart of §3-1. There the
...
%      먼저 아는 것이 순서이며, 게인을 정하는 §3-3 이 이 두 수를 쓴다.
%
...
%      controller is the order of work, and §3-3 designs its gains from these
%      two numbers.
%
%  만드는 것 / what it produces
%      표 하나와 그림 하나 : img/W03_result_openloop.png
%      One table and one figure: img/W03_result_openloop.png
%
...
V = W03_vars;      % every variable the model needs
c = W03_cols;      % names for the log columns — no magic numbers below
...
fprintf('\n  W03 section C — open-loop identification (loop_closed = 0)\n\n');
...
fprintf(['\n    THE DC GAIN IS EXACT, AND THAT IS THE EASIER OF THE TWO.\n' ...
         ...
         '\n      Section 3-1 reduced six degrees of freedom to one and kept only\n' ...
         ...
         '      what matters for the design of 3-3 is that the discrepancy is a\n' ...
...
%% ---- the figure --------------------------------------------------------
f = lab_fig('W03 C  open loop', 900, 380);
...
sgtitle('W03 C — the plant, identified from the plant', 'FontWeight','bold');
exportgraphics(f, fullfile(here,'img','W03_result_openloop.png'), 'Resolution', 150);
