...
summary: The first closed loop on the vessel — a type 0 plant, the error proportional control cannot remove, and what saturation does to an integrator
status: done
---
...
> - From Week 2: what each of $K_p$, $K_i$ and $K_d$ does on its own, the filtered derivative $N s/(s+N)$, back-calculation anti-windup, and the tuning order — all met on a single transfer function. This week puts the same controller around the Otter's surge axis, where the plant is no longer chosen for convenience.
> - From Week 1: $T_u = 1.1025$ s and $K_u = 0.012894$ (m/s)/N, and the propeller curve $T = k\,n|n|$.
> - Two numbers, and nothing else, are carried in from **Appendix A1**: the surge force this vessel can produce lies in $X \in [-133.42,\ 239.36]$ N. That limit is what causes the windup of §3-6. Appendix A1 derives it; a reader who takes the two numbers on trust loses nothing this week.
...
| | Question | Sections |
|---|---|---|
| 1 | **What does the surge axis look like to a controller?** One gain and one time constant, derived from the full model | 3-1 |
| 2 | **What does each term of a PID buy, and what does it cost?** P leaves an error; I removes it; D is the one that behaves differently here than it will in Week 4 | 3-2 to 3-4 |
| 3 | **What breaks when the actuator saturates?** Windup, and why an anti-windup scheme is not an optional extra | 3-5 to 3-7 |

**The number the week turns on** is $K_u = 0.012894$ (m/s)/N. Once the plant is two numbers, every steady-state claim in 3-2 and 3-3 is arithmetic rather than simulation.

**The one thing to carry into Week 4:** the derivative term sits beside the **mass** here, and it will sit beside the **damping** there. The term does not change; the axis does.
...
- Those two numbers are what the rest of the week argues with. Once the plant is $K_u$ and $T_u$, every steady-state claim in §3-2 and §3-3 is arithmetic rather than simulation.
...
> There is no $1/s$ anywhere in $u(s)/X(s)$. The loop is therefore **type 0**, and every consequence of §3-2 follows from that single structural fact rather than from any numerical value.
...
- Raising the gain shrinks the error but never removes it, and it does so at a cost. The gain also multiplies measurement noise and pushes the demanded force toward the saturation limits of §3-6.
...
- Matching the denominator to $s^2 + 2\zeta\omega_n s + \omega_n^2$ gives the design equations used in `W03_0_setup.m`:
...
| Symbol | Quantity | Value · source |
|---|---|---|
| $c_d$ | setpoint weight of the derivative term, dimensionless | $0$ in this week, `W03_0_setup.m` |
| $N$ | derivative filter bandwidth | $20$ rad/s, `W03_0_setup.m` |
...
>
> Stating it this way also removes a claim that cannot be sustained. The minus is sometimes explained as the textbook plus rewritten in terms of the measurement, on the grounds that $\dot e = \dot u_d - \dot u$ reduces to $-\dot u$ when the setpoint is held constant. That reasoning holds only while $\dot u_d = 0$. It says nothing about a setpoint that moves, and from Week 5 onward the command is produced by a guidance law and moves at every instant.
...
| moving continuously, as from the reference model of Week 8 or the guidance law of Week 5 | finite and non-zero | differ permanently by $K_d\dot u_d$ |
...
- The model carries $c_d$ as a variable rather than as a wiring decision, which makes the equivalence testable. Setting $c_d = 1$ and $K_d = 60$ brings the hand-built controller of §3-7 to within $4\times10^{-16}$ m/s of Simulink's PID Controller block over the whole run; at $c_d = 0$ the same two differ by $0.70$ m/s. The library block is the $c_d = 1$ member of this family, and §G measures both.
...
![The ideal derivative against the pseudo-derivative](../figures/w03-pseudo-derivative.svg)
...
>
> A bisection puts the threshold at $K_d = 90.13$ kg, $5.4\%$ above $M_{11}$; the offset is the derivative filter, which delivers slightly less than $K_d$ at the frequencies that matter. The reversed sign is still a defect at every gain — it moves the poles somewhere they were not designed to be — but the claim that it destabilises the loop on its own does not survive the arithmetic. `verify_w03_derivative` produces this table.

- This is a property of the **axis**, not of PID. Week 4 controls a heading, whose derivative is a rate rather than an acceleration, and there the same term supplies genuine damping. The lesson is to substitute the control law into the equation of motion before assuming what a term does.
...
- The equal split is forced: a pure surge demand contains nothing that distinguishes the two propellers. Week 6 treats the case where the demand does distinguish them and the split is no longer obvious.

> [!warning] $X_{\text{sat}}$ must leave the allocation block
> Without it the controller has no way of knowing that the actuator has stopped following the demand. Every anti-windup scheme in §3-6 is built on the difference $X_{\text{cmd}} - X_{\text{sat}}$, and a block that does not report what it delivered makes all of them impossible.

## 2-6. Windup is a saturation problem

### Why anti-windup is needed, before any equation

The integrator of §3-4 is a **memory of past error**. Every second the vessel runs slow, the integrator remembers it, and that memory is what eventually removes the offset a proportional term cannot. This is the whole reason the term exists, and it works because the memory is always *acted upon*: the integrator asks for more force, the force arrives, the error shrinks, and the memory stops growing.
...
![Where the loop breaks, and how far outside the limit the demand went](../figures/w03-windup.svg)
...
![Clamping and back-calculation, side by side](../figures/w03-antiwindup-two.svg)
...
- Reading the last term on its own: the excess is driven by a first-order lag towards the fixed point of §3-6, with a time constant
...
> Both schemes exist because the loop was opened by the actuator. The correct engineering response to persistent saturation is a reference the vessel can actually follow — which is what Week 8's reference model provides. Anti-windup limits the damage; it does not make an unreachable setpoint reachable.

## 2-7. The same controller, written twice

- Simulink ships a **PID Controller** block that contains everything §3-3 to §3-6 describes: three terms, a filtered derivative, an output limit, and the same two anti-windup schemes as dialog options.
...
- Both schemes of §3-6 are already inside the library block. Neither has to be built: they are chosen from one menu, on the **PID Advanced** tab.
- The menu is only active once **Limit output** is ticked on that tab. This is not a quirk — a controller with no saturation cannot wind up, so there is nothing for the scheme to act on, exactly as the $f(0,e) = 0$ requirement of §3-6 said.

![Selecting clamping or back-calculation in the PID Controller block](../figures/w03-pid-dialog.svg)
...
| (a) $K_b$ field | appears only for back-calculation, and is the $K_{\text{aw}}$ of §3-6 under Simulink's name |
| (b) dashed box | the same place in the dialog with clamping selected — the field is simply not there |

The dialog makes the structural difference of §3-6 visible before anything is run: back-calculation has a number to choose and clamping has none. A scheme that steers the integrator needs to be told how hard to pull; a scheme that switches it off does not.

### Is it the same algorithm?

Yes, and the model measures it rather than asserting it. Section H builds back-calculation **by hand** from the equation of §3-6 and runs it beside the library block on the same plant, with the same gains and the same limit:
...
- The two rows agree to $0.00\text{e}{+}00$ — not closely, but **bit for bit**. The equation printed in §3-6 is therefore a correct description of what the block computes, and not merely a scheme with the same name.
...
> A library block is a claim in a manual. Building the same thing from the equations and getting the same numbers turns that claim into something checked. It also settles the more useful question in the other direction: since the hand-built path matches, the equations of §3-6 can be trusted on a platform that has no PID block at all.
...
- To restore a model that has been broken: `W03_1_build_surge_control`.
...
|---|---|---|---|
| 0 | `W03_0_setup.m` | A | the base workspace. **The only file to edit this week** |
| 1 | `W03_1_build_surge_control.m` | B | `W03_surge_control.slx` and its block diagram |
| C | `W03_C_identify_plant.m` | C | `img/W03_result_openloop.png` |
| D | `W03_D_proportional_only.m` | D | `img/W03_result_P.png` |
| E | `W03_E_integral_and_derivative.m` | E | `img/W03_result_PI.png`, `img/W03_result_D.png` |
| F | `W03_F_windup.m` | F | `img/W03_result_windup.png` |
| G | `W03_G_block_vs_handbuilt.m` | G | `img/W03_result_block.png` |
| H | `W03_H_build_antiwindup.m` then `W03_H_antiwindup_run.m` | H | `img/W03_result_aw_principle.png` |
| I | `W03_I_build_pseudo_derivative.m` then `W03_I_pseudo_derivative_run.m` | I | `img/W03_result_pd.png` |
...
- The remaining files — `W03_vars.m`, `W03_cols.m`, `W03_plot.m`, `W03_aw_plot.m`, `W03_pd_plot.m`, `W03_animate.m` — are called **by** the section scripts and by the model. They are never run by hand.

## B. Reading the model (15 min)

![Block diagram of the surge speed loop](W03_simulink/img/W03_surge_control.png)
...
> Without knowing what the actuator delivered, the controller has no way of telling that the demand was not followed. Every anti-windup scheme in §3-6 is built on the difference $X_{\text{cmd}} - X_{\text{sat}}$, so an allocation block that does not report what it produced makes all of them impossible.
...
| `c_d` and `D input` | form $c_d u_d - u$, the signal the derivative acts on (§3-4) |
| `D filter` | $K_d N s/(s+N)$, applied to that signal |
| `anti-windup` | `aw_mode` selects one of the three schemes of §3-6 |
...
- All three are in `W03_surge_control.slx`, and `aw_mode` chooses. Opening `Surge controller` and then `anti-windup` shows them side by side:

![Inside the anti-windup subsystem: the three schemes as three boxes](W03_simulink/img/W03_antiwindup_inside.png)
...
> A single MATLAB Function would express all three schemes in nine lines and leave a tidier drawing. It would also put the one thing §3-6 is about — how the three differ — behind a double-click, where a reader comparing them has to read code rather than follow signals.
>
...
> A model can be self-consistent and still wrong. `verify_w03_antiwindup` therefore checks the three schemes twice.
>
...
>
> The second check does not use Simulink at all. The PI law of §3-3, the anti-windup of §3-6, the allocation of §3-5 and `otter.m` are written out again in plain MATLAB, straight from the equations in these notes, and integrated with the same fixed-step RK4 at the same step size. The two implementations are then compared on the whole trajectory:
>
...
>
> This is a third reason to prefer back-calculation, and it is not one §3-6 derived. Clamping stops the integrator; back-calculation steers it; and now — clamping makes the answer depend on the solver, while back-calculation does not. `verify_w03_antiwindup` produces this table, and fails if the clamping column ever stops shrinking, because that would mean the two really are different rules after all.
...
> | | |
> |---|---|
> | script | `W03_C_identify_plant.m` |
> | model | `W03_surge_control.slx` |
> | figure | `img/W03_result_openloop.png` |
>
> ```matlab
> W03_0_setup                    % once per session
> W03_C_identify_plant
> ```
>
...
![Open-loop identification](W03_simulink/img/W03_result_openloop.png)
...
- What is left over belongs to the degrees of freedom §3-1 discarded. The plant being measured keeps all six, and a surge force applied away from the centre of gravity also excites heave and pitch. An exact match was therefore never available. What matters for the design of §3-3 is that the gain, which fixes the steady state, is exact, and that the time constant is wrong by under two per cent.
...
> | | |
> |---|---|
> | script | `W03_D_proportional_only.m` |
> | model | `W03_surge_control.slx` |
...
![Proportional control](W03_simulink/img/W03_result_P.png)
...
> | model | `W03_surge_control.slx` |
> | figure | `img/W03_result_PI.png and img/W03_result_D.png` |
>
> ```matlab
> W03_0_setup                    % once per session
...
![PI design and the PI zero](W03_simulink/img/W03_result_PI.png)
...
![Derivative action on a velocity loop](W03_simulink/img/W03_result_D.png)
...
Week 4 runs the identical term in an identical PID controller and gets the opposite result. The difference is not the controller and not the tuning. On a **velocity** loop the derivative of the controlled variable is an acceleration, and acceleration multiplies mass. On an **angle** loop it is a rate, and rate multiplies damping. **The term did not change; the axis did.**
...
> | | |
> |---|---|
> | script | `W03_F_windup.m` |
> | model | `W03_surge_control.slx` |
> | figure | `img/W03_result_windup.png` |
>
> ```matlab
> W03_0_setup                    % once per session
> W03_F_windup
...
![Windup and three remedies](W03_simulink/img/W03_result_windup.png)
...
> | | |
> |---|---|
> | script | `W03_G_block_vs_handbuilt.m` |
> | model | `W03_surge_control.slx` |
...
![Two implementations of one controller](W03_simulink/img/W03_result_block.png)
...
- With $K_d = 0$ the two agree to **machine precision**. The assembled controller and the library block compute the same thing, and the back-calculation written by hand in §3-6 is the back-calculation the block implements.
- With $K_d = 60$ they differ by $0.70$ m/s at the peak. Neither is wrong, and §3-4 names the single parameter that separates them: the library block is the $c_d = 1$ case, which differentiates the **error**, so a step in $u_d$ passes through its derivative and produces a kick that the $c_d = 0$ path — differentiating the **measurement** — never sees.
- Setting `c_d = 1` in `W03_0_setup.m` closes the gap entirely. With $K_d = 60$ and $c_d = 1$ the two paths agree to $4\times10^{-16}$ m/s over the whole run, which is the strongest available statement that they are one controller written twice: the difference measured in the right panel is not an implementation difference at all, but a difference of setpoint weight.

**What the figure says**

The same plant, the same gains and the same setpoint, controlled twice — once by a controller assembled term by term from the equations of §3-4, once by Simulink's PID Controller block. The two panels differ only in whether the derivative term is switched on.
...
> | | |
> |---|---|
> | script | `W03_H_antiwindup_run.m` |
> | model | `W03_H_antiwindup.slx` |
> | figure | `img/W03_result_aw_principle.png` |
>
> ```matlab
> W03_0_setup                    % once per session
> W03_H_antiwindup_run
> ```
>
> Opening the model and pressing **Run** produces the same figure: the
> scopes update as it runs and `StopFcn` draws the summary at the end.
...
- Franklin's back-calculation gain is $K_a = 10$, drawn **inside** the integral gain. §3-6 shows that this is $K_{\text{aw}} = k_I K_a = 40$ in the form used on this page, and the model is given the $40$.
...
```matlab
W03_H_build_antiwindup
W03_H_antiwindup_run
```

![The anti-windup demonstration model](W03_simulink/img/W03_H_antiwindup.png)
...
![The principle, on the example of Franklin 8E Fig. 9.22](W03_simulink/img/W03_result_aw_principle.png)
...
- Rows 3 and 4 agree to **exactly zero**. The library block and the hand-built path are the same algorithm, so the theory of §3-6 is a correct description of what the block does.
...
> | | |
> |---|---|
> | script | `W03_I_pseudo_derivative_run.m` |
> | model | `W03_I_pseudo_derivative.slx` |
> | figure | `img/W03_result_pd.png` |
>
> ```matlab
> W03_0_setup                    % once per session
> W03_I_pseudo_derivative_run
> ```
>
> Opening the model and pressing **Run** produces the same figure: the
> scopes update as it runs and `StopFcn` draws the summary at the end.

- §3-4 found that on the surge axis the derivative term acts as a **mass** and makes things worse, so nothing in sections A to G shows what the derivative filter is *for*.
...
```matlab
W03_I_build_pseudo_derivative
W03_I_pseudo_derivative_run
```

![The pseudo-derivative model](W03_simulink/img/W03_I_pseudo_derivative.png)
...
![One derivative, taken four ways](W03_simulink/img/W03_result_pd.png)
...
- Week 4 controls heading, where the derivative term supplies genuine damping and is therefore indispensable. Its input is a yaw rate from an IMU, which is a noisy measurement.
...
- [ ] `W03_0_setup` printed $\zeta = 0.7000$ and $\omega_n = 1.5000$
- [ ] each of `W03_C_…` through `W03_G_…` ran on its own and wrote its figure into `W03_simulink/img/`
- [ ] `W03_H_antiwindup_run` and `W03_I_pseudo_derivative_run` completed and wrote four more
...
The second hour of the Week 3 session is spent building, in Simulink, the loop this lecture built from a script. Three problems, one hour, with a checker that compares the result against the numbers measured above.
...
- **Submit**: the modified `W03_0_setup.m`, a derivation, the numbers requested below, and two figures
...
| The integrator output is identically zero | an input of the `anti-windup` block is unconnected, so `Ki` arrives as 0 | rebuild with `W03_1_build_surge_control` |
...
| Adding $K_d$ makes the response worse | expected on this axis | $K_d$ adds to $M_{11}$; see §3-4 |
...
| The response is unchanged when `Kp` is edited in the block dialog | the block reads the workspace variable | edit `W03_0_setup.m` and run it |
...
| The library block and the hand-built path disagree at $K_d > 0$ | expected | the block differentiates the error, the hand-built path the measurement; see §3-7 |
...
| `W03_H_antiwindup_run` reports a recovery equal to the run length | the run ended before the loop recovered | lengthen `aw_T`; the default $60$ s is enough for $K_{\text{aw}} \ge 0.2$ |
...
- Franklin, G. F., Powell, J. D. and Emami-Naeini, A. *Feedback Control of Dynamic Systems*, 8th ed. Pearson, 2019. Fig. 9.22 draws back-calculation with the gain inside the integral gain, and Figs. 9.23–9.24 give the response with and without it. §3-6 relates the two forms.
- MSS toolbox, `Tools/MSS/VESSELS/otter.m` — the damping and saturation constants, lines 65 and 91–98.

### Course files

- `W03_simulink/W03_1_build_surge_control.m` — the model generator
- `W03_simulink/W03_0_setup.m` — the parameters, including the PI design equations
- `W03_simulink/W03_C_identify_plant.m` … `W03_G_block_vs_handbuilt.m` — one script per lecture section
- `W03_simulink/W03_plot.m` — the summary figure, called by both the runner and the model's `StopFcn`
- `W03_simulink/W03_animate.m` — the live view
- `W03_simulink/W03_H_build_antiwindup.m` — the standalone demonstration model of §H
- `W03_simulink/W03_H_antiwindup_run.m`, `W03_aw_plot.m` — section H, its experiments and figures
...
- The heading axis contains a free integrator, $\psi = \int r$, so the loop is **type 1** and proportional action alone leaves no steady-state error. Everything §3-2 established is reversed by one structural change.
...
- Preparation: bring $M_{66} = 42.65$ kg·m² and $N_r = -42.65$ from Week 1, and the finding of §3-4 about where a derivative term goes.
