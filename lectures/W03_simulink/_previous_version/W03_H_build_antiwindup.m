function W03_H_build_antiwindup()
%W03_H_BUILD_ANTIWINDUP  와인드업의 원리만 남긴 모델 W03_H_antiwindup.slx 를 만든다.
%                        Generate W03_H_antiwindup.slx, the principle on its own.
%
%   실행 / to run
...
%       W03_surge_control.slx 는 와인드업을 선체 위에서 보인다. 그런데 거기서는
%       플랜트가 열두 개의 상태를 갖고, 포화는 제곱 법칙을 따르는 프로펠러 곡선
%       이며, 설정값은 속도이다. 그중 어느 것도 이 절의 주제가 아니다. 주제는
%       1차 플랜트, 한계, 그리고 그 한계를 모르는 적분기 셋뿐이다.
%
%       W03_surge_control.slx shows windup on the vessel, where the plant has
...
%   the library block is doing what §3-6 says it does.
%
%   Regenerating is safe: any existing W03_H_antiwindup.slx is overwritten.
...
%  and its state is therefore a signal that can be plotted. Everything §3-6
...
add_block('simulink/Sinks/To Workspace', [q '/W03aw'], ...
          'VariableName','W03aw', 'SaveFormat','Structure With Time', ...
...
row_feed(q, 'log', QIN);
add_line(q, 'log/1','W03aw/1','autorouting','on');
...
'WEEK 3, THE ANTI-WINDUP PRINCIPLE ON ITS OWN'
''
...
set_param(m, 'StopFcn', 'W03_aw_plot;');
