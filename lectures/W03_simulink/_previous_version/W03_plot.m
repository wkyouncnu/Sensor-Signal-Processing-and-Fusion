...
%W03_PLOT  W03 실행 결과의 설정값·응답·요구한 힘·적분기 상태를 그린다.
%          Reference, response, demand and integrator state of Week 3 runs.
%
%   W03_plot                      기본 작업공간에 있는 실행 결과
%                                 the run sitting in the base workspace
%   W03_plot(R, LBL, ttl)         실행 결과들의 셀 배열 / a cell array of runs
%   f = W03_plot(...)             그림 핸들 / the figure handle
%
...
%   [u v r N E psi u_d X_cmd X_sat I n1]. w03_cols reorders it into the six
...
if nargin < 1 || isempty(R)
    ...
    LBL = {'run'};
    ttl = 'W03 surge speed control — result of the last simulation';
end
if nargin < 2 || isempty(LBL), LBL = arrayfun(@(k) sprintf('run %d', k), ...
                                              1:numel(R), 'UniformOutput', false); end
if nargin < 3 || isempty(ttl), ttl = 'W03 surge speed control'; end
...
f = lab_fig('W03  speed loop', 1150, 680);
