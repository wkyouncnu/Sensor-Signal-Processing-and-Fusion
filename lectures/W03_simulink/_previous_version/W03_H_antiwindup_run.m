%% W03 · 절 H — 와인드업의 원리를, 선체를 치우고 1차 플랜트 위에서
%  W03 · Section H — the windup principle, on a first-order plant
%
...
%      Part 2 절 H 이며, §3-6 과 절 F 가 선체에서 보인 것을 가장 단순한 플랜트
%      위에서 다시 보이는 자리이다.
%      This is section H of Part 2. What §3-6 derived and section F observed on
...
%      적분게인 안쪽에 두므로 K_aw = kI Ka = 40 이다. §3-6 이 그 관계를 다룬다.
...
%      §3-6 works through the relation.
%
...
%      img/W03_H_antiwindup.png           블록도 / the block diagram
%      img/W03_result_aw_principle.png    네 방식을 한 플랜트 위에 / the four
%                                         schemes on one plant
%
%  변수를 바꾸어 돌리려면 / to override a variable
%      W03_H_antiwindup_run 은 스크립트이므로 varargin 을 아래에서 비워 둔다.
...
if ~isfile(fullfile(here,'W03_H_antiwindup.slx')), W03_H_build_antiwindup(); end
...
fprintf('\n  W03 · the anti-windup principle, on the example of Franklin 8E Fig. 9.22\n');
fprintf('\n    G(s) = 1/s,  u in [%g, %g],  kp = %g, kI = %g\n', ...
        V.aw_umin, V.aw_umax, V.aw_Kp, V.aw_Ki);
fprintf('    Ka = %g in the source, which is K_aw = kI Ka = %g here  (see 3-6)\n', ...
        V.aw_Ka, V.aw_Kb);
...
load_system('W03_H_antiwindup');
print('-sW03_H_antiwindup', '-dpng', '-r150', img('W03_H_antiwindup.png'));
close_system('W03_H_antiwindup', 0);

f = W03_aw_plot(R, ...
    'W03 — one plant, one limit, four ways of treating the integrator', R0);
exportgraphics(f, img('W03_result_aw_principle.png'), 'Resolution', 150);

%  WITHDRAWN 2026-09-08, at the lecturer's request: the two-panel Kb sweep
%  figure that used to be saved here as img/W03_result_aw_Kb.png.
%
...
%  measurement provenance is not what §10-8 asks to cut.

fprintf('\n  figure written to %s\n\n', img('W03_result_aw_principle.png'));
%  (the old function-closing end was here; this file is a script now)

% =========================================================================
function o = one(V, varargin)
for i = 1:2:numel(varargin), V.(varargin{i}) = varargin{i+1}; end
in = Simulink.SimulationInput('W03_H_antiwindup');
fn = fieldnames(V);
for i = 1:numel(fn), in = in.setVariable(fn{i}, V.(fn{i})); end
evalc('r = sim(in);');
y = squeeze(r.W03aw.signals.values);  if size(y,1) < size(y,2), y = y.'; end
o.t = r.W03aw.time;
...
%  그의 Ka 는 이 강의의 K_aw 와 같은 수가 아니다 (§3-4 · §3-6):
