%% W03 · 절 F — 와인드업, 그리고 그것을 견디는 두 가지 방법
%  W03 · Section F — windup, and two ways of surviving it
%
...
%      Part 2 의 절 F 이며 §3-6 을 받는다. 절 E 까지의 실험은 모두 작동기가
%      요구를 들어줄 수 있는 범위 안에서 이루어졌다. 이 절은 처음으로 들어줄 수
%      없는 요구를 하고, 그때 적분기에서 무슨 일이 일어나는지를 본다.
%
%      This is section F of Part 2, following §3-6. Every experiment up to
...
%      표 하나와 img/W03_result_windup.png
%      One table and img/W03_result_windup.png
...
fprintf('\n  W03 section F — windup: u_d = 3.5 m/s (unreachable) for 35 s, then 1.5\n\n');
fprintf('    %-18s %14s %16s %18s\n', ...
        'anti-windup', 'peak I [N]', 'peak X_cmd [N]', 'recovery [s]');
fprintf('    %s\n', repmat('-', 1, 72));

for i = 1:3
    RW{i}    = run_sim('W03_surge_control', V, ...
...
%% ---- the figure --------------------------------------------------------
f = lab_fig('W03 F  windup', 1150, 760);
...
sgtitle('W03 F — windup is a saturation problem wearing an integrator costume', ...
        'FontWeight','bold');
