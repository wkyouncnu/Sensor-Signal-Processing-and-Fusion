...
3. Explain why the derivative term damps this loop and did not damp the loop of Week 3, in terms of where each one lands in the equation of motion.
...
| Course folder | `GradCourse/lectures/W04_simulink` |
...
Six sections. The week is deliberately the **same question as Week 3, asked about a different axis**, and most of it is about why the answers come out opposite.

| | Question | Sections |
|---|---|---|
| 1 | **How is the yaw axis different from the surge axis?** It carries a free integrator, and that changes every steady-state answer | 4-1 |
| 2 | **What law, and where does the derivative go this time?** P–D on the yaw rate, not on the error | 4-2, 4-3 |
| 3 | **Two things that only appear on an angle.** The vessel does not travel where it points, and $+180°$ and $-180°$ are the same heading | 4-4, 4-5 |
| 4 | **Two demands into two propellers** | 4-6 |

**The structural fact the week rests on** is $\psi = \int r$. Week 3's plant had no free integrator and could not reach its setpoint at any gain; this one reaches it at every gain. Nothing was tuned to make that happen.

**The debt this week leaves unpaid** is 4-4: the crab angle means the vessel does not go where it points. Week 5 has to steer around it.
...
> Week 3's plant had no integrator, so the loop was **type 0** and proportional control could never reach the setpoint. This plant has one, so the loop is **type 1** and proportional control reaches the setpoint at every gain. Nothing was tuned to achieve that. It is a property of the axis.

- The reason is the same physical argument as Week 3, run backwards. Holding a steady **speed** needs a steady force, because damping never stops. Holding a steady **heading** needs no moment at all, because a vessel that is not turning has no yaw damping to overcome. A proportional controller can supply zero moment at zero error, and that is exactly what is required.

| | Week 3, surge | Week 4, heading |
|---|---|---|
...
- The Otter has no rudder. Its yaw moment comes from the **difference between two propellers**, so $\delta$ is replaced by $\tau_N$ and the same first-order form appears directly from §4-1:
...
| Choice | Reason |
|---|---|
| $\operatorname{ssa}$ on the error | angles wrap; §4-5 is what happens without it |
...
| $c_d = 0$ this week | the command is a step, so $r_d$ is zero except at the step itself, where it is not differentiable. The port exists so that Week 8 can fill it |
...
![How ζ and ωₙ change a second-order response](../figures/w04-second-order.svg)
...
> [!important] Why the textbook overshoot formula is trustworthy here and was not in Week 3
...
![The wrap: two headings 20° apart, and the error that says −340°](../figures/w04-ssa.svg)
...
- The controller asks for a yaw moment. A constant forward force $X_{\text{ff}}$ is added so that the vessel travels while it turns and the track is worth looking at. Week 3 had one demand and two propellers and split the force in half; this week there are **two** demands, and the split is no longer arbitrary.
...
- Equivalently, and this is the form Week 6 generalises, the result is the matrix inverse:
...
- The map is **square** in the $(X, N)$ plane and $\det\mathbf{B} = -2y_{\text{pont}} = -0.79 \neq 0$, so the inverse exists and is unique. Nothing is optimised and nothing is chosen — there is exactly one thrust pair for each demand pair. Week 6 meets the case where there are more thrusters than demands, $\mathbf{B}$ is no longer square, and a choice genuinely has to be made.
...
```matlab
cd GradCourse/lectures/W04_simulink
W04_0_setup
```
...
- `W04_0_setup.m` is **the only file to edit this week**. To restore a broken model: `W04_1_build_heading`.

**The files of this week, in the order the sections use them**

| Order | File | Section | What it produces |
|---|---|---|---|
| 0 | `W04_0_setup.m` | A | the base workspace, so the model can be run from Simulink |
| 1 | `W04_1_build_heading.m` | B | `W04_heading_control.slx` and `img/W04_heading_control.png` |
| C | `W04_C_proportional_only.m` | C | `img/W04_result_P.png` |
| D | `W04_D_derivative_action.m` | D | `img/W04_result_D.png` |
| E | `W04_E_the_wrap.m` | E | `img/W04_result_ssa.png` |
| F | `W04_F_big_turns_overshoot_less.m` | F | `img/W04_result_size.png` |
...
- The remaining files — `W04_vars.m`, `W04_read.m`, `W04_plot.m`, `W04_animate.m` — are called **by** the scripts above and by the model. They are never run by hand.
...
> `W04_1_build_heading` writes `W04_simulink/img/W04_heading_control.png` at the end of the build. The diagram belongs to the builder and to nothing else, so it changes when the model changes and not when a gain changes.

![Block diagram of the heading loop](W04_simulink/img/W04_heading_control.png)
...
| `Control allocation` (sand) | $(\tau_N, X_{\text{ff}}) \to$ two shaft speeds, by the inverse of §4-6 |
| `Otter USV` (green) | `otter.m`, called unchanged |
| `Measurements` (grey) | selectors, scope, workspace log and live view |

- The chain is the same one as Week 3 with the controller replaced. **One signal travels backwards**: the state $\mathbf{x}$, from which the autopilot selects $\psi$ and $r$.
...
> [!note] To produce this figure
> `W04_C_proportional_only.m` runs `W04_heading_control.slx` three times with `Kd = 0`, prints the table below, and writes `img/W04_result_P.png`.
...
![Proportional control on a type 1 plant](W04_simulink/img/W04_result_P.png)
...
- The steady-state error is **zero at every gain**, to the tolerance of the solver. Week 3's table at the same place read $43.68$, $13.43$ and $3.73$ per cent.
...
Look at the left panel first. All three headings reach $60°$ and stay there — **with no integral term anywhere in the controller.** Week 3 needed one and still had to work for it. Here a plain proportional gain lands exactly on the setpoint, and it does so at $K_p = 30$, $100$ and $300$ alike.
...
So the bargain is different from Week 3's. There, gain bought accuracy the loop could not otherwise have. Here accuracy is free and gain buys only speed, until the actuator runs out. Section D adds the term that lets $K_p$ rise further without the response starting to ring.
...
> [!note] To produce this figure
> `W04_D_derivative_action.m` runs `W04_heading_control.slx` four times with `Kp` held and `Kd` swept, prints the table below, and writes `img/W04_result_D.png`.
...
![Derivative action on a heading loop](W04_simulink/img/W04_result_D.png)
...
- The **trend** is exactly as predicted: more $K_d$, more damping, less overshoot. Compare Week 3's table, where more $K_d$ meant *more* overshoot.
...
Derivative action works here. Overshoot falls from $11.74\%$ at $K_d = 0$ to nothing at $K_d = 74.9$ — the exact opposite of Week 3, where the same term made the same kind of controller worse.
...
and $K_d$ lands **beside the damping**. In Week 3 the controlled variable was a velocity, so its derivative was an acceleration, and the identical term landed beside the *mass* instead. **The term did not change; the axis did.**
...
> The design equations of §4-3 use $N_r$ alone. The hull's actual damping is $N_h = N_r(1 + 10|r|)r$, which is larger whenever the vessel is turning quickly. A small step keeps $|r|$ small and the linear prediction close. Section F makes the same nonlinearity the subject rather than a nuisance.
...
> [!note] To produce this figure
> `W04_E_the_wrap.m` runs `W04_heading_control.slx` twice — once with `use_ssa = 1`, once with `use_ssa = 0` — prints the table below, and writes `img/W04_result_ssa.png`.
...
![The wrap, with and without ssa](W04_simulink/img/W04_result_ssa.png)
...
> [!note] To produce this figure
> `W04_F_big_turns_overshoot_less.m` runs `W04_heading_control.slx` four times with the gains held and the step size swept, prints the table below, and writes `img/W04_result_size.png`.
...
![Step size and the nonlinear damping](W04_simulink/img/W04_result_size.png)
...
The practical consequence is worth stating plainly. The design equations of §4-3 use $N_r$ alone, so they are a **small-signal** result: right for small corrections, and for large ones **conservative rather than optimistic**. A design that predicts $11.7\%$ overshoot and delivers $0.1\%$ has erred in the safe direction, which is the only direction worth erring in.
...
| 3 | Showed $K_d$ damping this axis | overshoot $11.74 \to 4.10 \to -0.02$ per cent as $K_d$ rises, the reverse of Week 3 |
...
- [ ] Able to explain, from the equation of motion, why $K_d$ damps here and did not in Week 3
- [ ] Able to write $\operatorname{ssa}$ and say where in the model it belongs

### Laboratory

- [ ] `W04_0_setup` printed $\zeta = 0.9000$ and $\omega_n = 1.5312$
- [ ] Sections C, D, E and F were run in that order and together wrote four result PNG files into `W04_simulink/img/`
...
The second hour of the Week 4 session is spent building, in Simulink, the loop this lecture built from a script.
...
- Problem 1 is deliberately the same question Week 3 asked and got the opposite answer to. The difference is $\psi = \int r$, and nothing else.
...
- **Submit**: the modified `W04_0_setup.m`, a derivation, the numbers requested below, and one figure
...
| Symptom | Cause | Fix |
|---|---|---|
| The vessel turns almost all the way round for a small heading change | `use_ssa = 0` | set `use_ssa = 1`; see §4-5 |
...
- `W04_simulink/W04_1_build_heading.m` — the model generator
- `W04_simulink/W04_0_setup.m` — the parameters, including the design equations
- `W04_simulink/W04_C_proportional_only.m` · `W04_D_derivative_action.m` · `W04_E_the_wrap.m` · `W04_F_big_turns_overshoot_less.m` — one script per laboratory section
- `W04_simulink/W04_vars.m` · `W04_read.m` — the same numbers as a struct, and the log with the angles in degrees
- `W04_simulink/W04_plot.m` — the summary figure, called by both the runner and the model's `StopFcn`
...
- This week was told what heading to hold; a human typed the number into a step block. Week 5 asks where that number comes from, and the answer is a list of waypoints and a guidance law.
...
- Preparation: §4-4 of this week, on the crab angle. Week 5 opens by quantifying exactly what it costs.
