function W05_P1_start(mdl)
%W05_P1_START  5주차 실습 문제의 출발 모델을 만든다.
%              Create the starting model for the Week 5 laboratory problems.
%
%   >> W05_P1_start                 W05_P1.slx 를 만든다 / creates W05_P1.slx
%   >> W05_P1_start('W05_P1_kim')   다른 이름으로 만든다 / another name
%
%   무엇을 주고 무엇을 주지 않는가
%   what this script gives, and what it withholds
%
%   It creates a complete Week 4 vessel — heading autopilot, control
...
%   autopilot changes between Week 4 and Week 5.
%
%   THE BLOCKS THAT ARE PROVIDED
%
%     Heading autopilot    tau_N = Kp ssa(psi_d - psi) - Kd r      (Week 4)
...
%   from W05_vars.m. The starting model wires a Constant of zero into the
%   autopilot so that it compiles before anything is built; replace it.
%
%   See also W05_CHECK, W05_0_SETUP.

if nargin < 1 || isempty(mdl), mdl = 'W05_P1'; end
...
w05_lab_vessel(mdl, 260);
add_line(mdl, 'psi_d placeholder/1', 'Heading autopilot/1', 'autorouting','smart');

a = Simulink.Annotation([mdl '/brief']);
a.Text = strjoin({ ...
'WEEK 5 LABORATORY  -  BUILD THE GUIDANCE LAYER YOURSELF'
''
'A complete Week 4 vessel is given. Only the COMMAND is missing.'
...
fprintf('\n  created %s\n', out);
fprintf('  open it, build Problem 1, then run:  W05_check(1, ''%s'')\n\n', mdl);
end
