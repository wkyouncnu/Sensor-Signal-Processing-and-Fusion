function pass = W05_check(problem, mdl)
%W05_CHECK  학생이 만든 5주차 모델을 돌려 강의의 측정값과 대조한다.
%           Run a student's Week 5 model and check it against the lecture.
%
%   W05_check(1)                W05_P1.slx 를 검사한다 / checks W05_P1.slx
%   W05_check(2, 'W05_P1_kim')  다른 모델을 검사한다 / checks another model
%   pass = W05_check(3, mdl)    모두 통과하면 true / true when every test passed
%
...
%
%   See also W05_P1_START, W05_0_SETUP.

if nargin < 2 || isempty(mdl), mdl = 'W05_P1'; end
...
fprintf('\n  W05 problem %d  —  checking model %s\n', problem, mdl);
fprintf('  ------------------------------------------------------------\n');
switch problem
    case 1, pass = check_p1(mdl);
    case 2, pass = check_p2(mdl);
    case 3, pass = check_p3(mdl);
    otherwise, error('W05_check:problem', 'problem must be 1, 2 or 3');
end
fprintf('  ------------------------------------------------------------\n');
...
if size(M,2) ~= w
    error('W05_check:width', '%s has %d columns; %d were expected.', ...
          name, size(M,2), w);
end
end
