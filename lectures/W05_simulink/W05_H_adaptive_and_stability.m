%% W05 · 절 H — 적응 게인, 그리고 그것을 정당화하는 함수
%  W05 · Section H — the adaptive gains, and the function that justifies them
%
...
%      한꺼번에 검사한다. §5-9-6 에는 그 유도가 실제로 무엇을 보이고 무엇을
%      보이지 못하는지가 적혀 있다.
%
...
%      tests both at once. §5-9-6 records what that derivation does and does
%      not establish.
%
%  만드는 것 / what it produces
%      표 하나와 img/W05_result_stability.png
%      One table and img/W05_result_stability.png
...
fprintf('\n  W05 section H — the two adaptive gains, in a %.2f m/s current\n', V.V_c);
fprintf('\n  1) ILOS: kappa, with Ki = kappa / Delta\n\n');
fprintf('    %8s %10s %16s %16s %16s\n', 'kappa', 'Ki', 'settled |y_e|', 'peak |y_e|', 'settling [s]');
fprintf('    %s\n', repmat('-', 1, 72));
for i = 1:numel(KK)
    y = W05_read(run_sim('W05_guidance', V, 'kappa', KK(i)));
    k = y.t >= 0.8*y.t(end);
    leg1 = round(y.wp(:,3)) == 1;
...
YG = cell(size(GG));
for i = 1:numel(GG)
    y = W05_read(run_sim('W05_guidance', V, 'gamma', GG(i)));
    YG{i} = y;
    k = y.t >= 0.8*y.t(end);
    leg1 = round(y.wp(:,4)) == 1;
...
%
%  The adaptation law was CHOSEN to make dV/dt negative; §5-9 does that
...
fprintf(['\n    V FALLS BY A FACTOR OF %.0f FROM ITS PEAK, AND IT IS NOT MONOTONE.\n' ...
         ...
         '    derivation of 5-9 predicts. The %.0f per cent of samples where V\n' ...
         '    rises are not a contradiction of it, because the derivation\n' ...
         '    assumes the heading follows psi_d EXACTLY. It does not: there is\n' ...
         '    a Week 4 autopilot in between, with its own settling time, and\n' ...
...
%% ---- the figure --------------------------------------------------------
f = lab_fig('W05 H  the adaptation, and the function that justifies it', 1000, 440);
COL = lines(numel(GG));
...
sgtitle('W05 H — the adaptation law is the choice that makes V decrease', ...
        'FontWeight','bold');
exportgraphics(f, fullfile(here,'img','W05_result_stability.png'), 'Resolution', 150);
fprintf('\n  figure -> img/W05_result_stability.png\n\n');
