%% W05 · 절 E — 전방주시거리 Delta 는 무엇과 무엇을 맞바꾸는가
%  W05 · Section E — what the look-ahead distance trades against what
%
...
%      Part 2 의 절 E 이며 §5-5 의 극한 논의를 수치로 채운다.
...
%      표 하나와 img/W05_result_lookahead.png
%      One table and img/W05_result_lookahead.png
...
fprintf('\n  W05 section E — the look-ahead distance, started 8 m off the path\n\n');
...
for i = 1:numel(DD)
    o    = run_sim('W05_guidance', V, 'Delta', DD(i));
    y    = W05_read(o);
    R{i} = y;
...
%% ---- the figure --------------------------------------------------------
f = lab_fig('W05 E  look-ahead', 1150, 470);
COL = lines(numel(DD));
...
sgtitle('W05 E — \Delta is the only number a LOS law has', 'FontWeight','bold');
exportgraphics(f, fullfile(here,'img','W05_result_lookahead.png'), 'Resolution', 150);
fprintf('\n  figure -> img/W05_result_lookahead.png\n\n');
