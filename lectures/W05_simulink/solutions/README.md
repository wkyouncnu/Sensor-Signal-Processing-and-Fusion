...
```matlab
cd lectures/W05_simulink/solutions
W05_S1_los                   % builds W05_S1.slx — both laws in one model
W05_S_expected               % regenerates the expected-result figures
...
All three pass and `check_overlaps('W05_S1')` is **0**.
...
|---|---|---|
| **One rotation, both errors** | $x_e^{\,p}$ and $y_e^{\,p}$ from the same matrix product | A distance formula plus a sign test gives $y_e^{\,p}$ and throws $x_e^{\,p}$ away. Section 5-6's switching test needs the along-track coordinate, and computing it separately invites the two to disagree |
...
It is the same reason the lecture's own `W05_guidance.slx` runs four laws through one autopilot bank, and the same reason Week 3 held three controllers in one model.
