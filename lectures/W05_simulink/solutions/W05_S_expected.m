function W05_S_expected()
%W05_S_EXPECTED  올바른 5주차 제출물이 내야 하는 결과 그래프를 만든다.
%                The result graphs a correct Week 5 submission must produce.
%
%   >> W05_S_expected
%
...
f = lab_fig('W05 P1 expected', 940, 380);
subplot(1,3,1);
...
save_png(f, fullfile(img,'W05_P1_expected.png'));
...
f = lab_fig('W05 P2 expected', 900, 380);
subplot(1,2,1);
...
save_png(f, fullfile(img,'W05_P2_expected.png'));
...
f = lab_fig('W05 P3 expected', 940, 380);
subplot(1,3,1);
plot(y.y, y.x, 'LineWidth',1.6); hold on; grid on; axis equal
...
save_png(f, fullfile(img,'W05_P3_expected.png'));
...
evalin(b, 'bdclose(''W05_S1'');');
load_system('W05_S1');
set_param('W05_S1','StopTime','T_final','ReturnWorkspaceOutputs','off');
evalin(b, 'sim(''W05_S1'');');
