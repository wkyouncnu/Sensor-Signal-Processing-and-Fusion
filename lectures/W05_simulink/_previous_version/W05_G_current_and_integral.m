%% W05 · 절 G — 조류, 그리고 그것을 이기는 두 가지 법칙
%  W05 · Section G — the current, and the two laws that beat it
%
...
%      Part 2 의 절 G 이며, 4주차 §4-4 가 남겨 둔 빚을 갚는 자리이다. 거기서
%      크랩각을 재면서 "경로추종 법칙은 이것 때문에 영구적인 오차를 남긴다" 고
%      예고했다. 이 절이 그 오차의 크기를 재고, 그것을 없애는 두 가지 방법을
%      나란히 놓는다.
%
%      This is section G of Part 2, and it settles the debt left by §4-4 in
%      Week 4, where the crab angle was measured and it was announced that a
...
%      1. 단순한 LOS 는 조류 아래에서 영구적인 경로이탈 오차를 남긴다. §5-7 이
%         그 크기를 미리 계산한다 : y_e = Delta tan(beta).
%      2. ILOS 는 적분 상태를 하나 두어 그것을 없앤다.
%      3. ALOS 는 크랩각 자체를 추정해서 없앤다.
%
%      1. Plain LOS settles on a permanent cross-track error, whose size §5-7
...
%      표 셋과 img/W05_result_current.png
%      Three tables and img/W05_result_current.png
...
fprintf('\n  W05 section G — a current of %.2f m/s at %.0f deg\n\n', ...
...
%  대지속력 U. 강의 5-9 기호표가 인용한다 — 2026-09-14 검토 전에는 거기 근거 없이
...
fprintf('    %s\n', repmat('-', 1, 70));
for i = 1:numel(VC)
    yy = W05_read(run_sim('W05_guidance', V, 'V_c', VC(i)));
...
%% ---- the figure --------------------------------------------------------
f = lab_fig('W05 G  current', 1250, 470);
...
sgtitle('W05 G — a current is a permanent offset, unless the law knows about it', ...
        'FontWeight','bold');
exportgraphics(f, fullfile(here,'img','W05_result_current.png'), 'Resolution', 150);
fprintf('\n  figure -> img/W05_result_current.png\n\n');


%% ---- the two states, filling up ----------------------------------------
%  강의 5-8-5a · 5-9-6a 의 표. 오토파일럿을 빼고 §5-8-6 의 오차동역학만 적분한다 —
