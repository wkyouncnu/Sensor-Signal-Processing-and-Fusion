...
**The model.** `W05_G_tuning` — the fullest model of the week. Everything except the `guidance` block is Week 4, unchanged and not retuned.
...
Measured by Experiment 5-5, below, a current flowing east across a path north:
...
Measured by Experiment 5-5 at 0.3 m/s, with $\Delta = 5$ m:
...
### Experiment 5-5 · A current, and ILOS (15 min)

**What it measures.** First lines 3 and 4, by sweeping the current under LOS and comparing the offset with $\Delta\tan$ of the heading held; then line 7, by sweeping $\kappa$ at one current.

**The models.** `W05_D_LOS` for the first table and `W05_F_ILOS` for the second — the same guidance block with the integral state switched off and on. One 400 m leg, so that the offset has time to settle.
...
open_system('W05_D_LOS')           % Run — 경로와 나란히, 2.1 m 옆으로 / parallel, 2.1 m beside it
open_system('W05_F_ILOS')          % Run — 같은 조류, 오차가 사라진다 / the same current, no offset
kappa = 1;                         % Run — 더 빨리 없애지만 더 넘어간다 / faster, and past it
W05_F_current_and_ILOS             % 조류 넷과 kappa 넷을 한 번에 / four currents and four kappa
```
...
![Experiment 5-5: LOS and ILOS in a 0.3 m/s cross current](W05_simulink/img/W05_result_current.png)
...
| What to try | What to watch |
|---|---|
| `V_c = 0.5;` on `W05_D_LOS` | $4.234$ m off, holding $-40.3°$ — and $5\tan 40.3° = 4.234$ m: line 4 holds at any current |
| `Delta = 2.5; V_c = 0.3;` on `W05_D_LOS` | half the look-ahead distance halves the offset, $1.054$ m at the same $-22.9°$ heading, exactly as line 4 says. It also sharpens the transient, which is why §5-6 fixes $\Delta$ on the transient and leaves the current to $\kappa$ |
...
- On 60 m legs $\kappa = 0.1$ is too slow. $\kappa = 0.3$ and $0.5$ are nearly equal; Experiment 5-5 showed a larger $\kappa$ overshoots more, so $\kappa = 0.3$.
...
> - **Ask** — "Why is $\kappa$ chosen on the mission and not on the long leg of Experiment 5-5?" Short legs leave little time to integrate; the gain that is best on 400 m is too slow on 60 m.
...
| 5-5 | `W05_D_LOS`, `W05_F_ILOS` | `W05_F_current_and_ILOS` | a current, and the integral |
...
| 5 | a current and ILOS | Experiment 5-5: LOS offset $= \Delta\tan$(heading) to the millimetre; ILOS removes it |
