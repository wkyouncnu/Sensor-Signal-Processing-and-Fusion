...
| Model | `lectures/W05_simulink/W05_guidance.slx`, built by `W05_1_build_guidance.m` |
...
- Weeks 1 to 4 were **told** what heading to hold. A human typed $60°$ into a step block and the autopilot delivered it. Nothing in those three weeks said where the number should come from.
- **Guidance** is where it comes from. Given a list of waypoints and the vessel's present position, guidance produces $\psi_d$; the autopilot of Week 4 then does what it already does. Guidance and control are separate layers, and this separation is the reason the Week 4 autopilot can be reused untouched.
...
| $x_{i+1}^n,\ y_{i+1}^n$ | the waypoint being approached | m — `WP_N`, `WP_E` in `W05_vars.m` |
| $\psi_d$ | commanded heading | rad — into the Week 4 autopilot |
...
![Aiming at a point against following a path](../figures/w05-atan2-vs-los.svg)
...
| | Question | Sections |
|---|---|---|
| 1 | **What is the path, and where is the vessel relative to it?** Two waypoints give an angle; one rotation gives two errors | 5-2, 5-3 |
| 2 | **What heading closes that error?** The LOS law, what $\Delta$ trades away, and when to move to the next leg | 5-4, 5-5, 5-6 |
| 3 | **Why is that not enough?** In a current the law settles *beside* the path and stays there — with the heading error already zero | 5-7 |
| 4 | **Two ways to fix it.** ILOS invents the missing term; ALOS estimates it | 5-8, 5-9 |

Sections 5-10 and 5-11 then set the equations against the code, and point at what lies beyond straight legs.

**Everything after 5-7 exists because of one number**: $\Delta\tan\beta$, the offset the plain law is left holding. A reader short of time can go 5-2 → 5-3 → 5-4 → 5-7 and still have the argument.
...
>
> **The code cannot carry superscripts.** MATLAB identifiers have no place to put them, so `_tools/crosstrack_err.m` returns `x_e` and `y_e`, and the Simulink signals are named the same way. Every such identifier means the path-frame quantity. §5-10 sets the equations and the code side by side.
...
> Lekkas and Fossen (2014) replace the straight leg by a Hermite spline and $\pi_p$ by the spline's tangent angle at the closest point. Everything downstream of $\pi_p$ in this week — §5-3 through §5-9 — is unchanged by that substitution. §5-11 gives the reference.
...
![The two errors from one rotation](../figures/w05-track-frame.svg)
...
$\hat{\mathbf{t}}$ is a unit vector at angle $\pi_p$ from North, which is the leg's direction by the definition of §5-2. $\hat{\mathbf{n}}$ is $\hat{\mathbf{t}}$ turned by $+90°$, which in a North-East frame points to **starboard** of the leg. The two are orthonormal, so the pair is a basis and the decomposition below is unique.
...
| | consumed by | because |
|---|---|---|
| $x_e^{\,p}$ | the **switching** logic of §5-6 | it says how much of the leg has been run, so $d_k - x_e^{\,p}$ is how much is left |
| $y_e^{\,p}$ | the **guidance law** of §5-4 and every law after it | it is the quantity that "follow the path" means to drive to zero |
...
![The LOS geometry](../figures/w05-los-geometry.svg)
...
> The layout and every symbol are those of Fossen's TTK 4190 lecture notes and of the *Handbook*, 2nd ed. (2021), §12.3, so that this course and the standard reference can be read against each other without translation. The geometry is computed by `_tools/w05_losgeo.m`, which checks that the perpendicular really is perpendicular and that the drawn line of sight really is at $\chi_d$ before it emits a single coordinate.

### The derivation

Let $F$ be the foot of the perpendicular from the vessel to the leg, and let the **aim point** $A$ lie a further distance $\Delta$ along the leg. Work in the path frame $\{p\}$ of §5-3, where the vessel sits at $(x_e^{\,p},\ y_e^{\,p})$ and, by construction, $F$ sits at $(x_e^{\,p},\ 0)$ and $A$ at $(x_e^{\,p} + \Delta,\ 0)$.
...
|---|---|---|---|
| $\pi_p$ | path-tangential angle | rad | §5-2, from the two waypoints |
| $y_e^{\,p}$ | cross-track error, resolved in $\{p\}$ | m | §5-3, from the rotation |
| $\Delta$ | look-ahead distance, measured **along the path** | m | $8$ m — chosen in §5-5, justified by the sweep of section E |
| $K_p$ | proportional gain, $1/\Delta$ | $\text{m}^{-1}$ | **not free** — fixed by $\Delta$. §5-8 names it, and it is the same number |
...
| $\psi_d$ | commanded **heading** | rad | what this course sends to the Week 4 autopilot |

> [!important] The law commands a course; this course commands a heading
> Fossen writes the law with $\chi_d$ on the left, because the line of sight is the direction the vessel should **travel** in. What Week 4 built is a **heading** autopilot, so from here on this course writes
>
...
> **The substitution is not free, and this week measures its price.** §5-7 shows that using $\psi_d$ in a current leaves the vessel permanently $\Delta\tan\beta$ to one side of the path; §5-8 and §5-9 are two ways of paying it back.
...
- The two degenerate limits are the two ends of §5-5's trade, and section E measures both of them.
...
![The look-ahead distance](../figures/w05-lookahead.svg)
...
> [!tip] The third cost of a large $\Delta$ appears in §5-7
...
![Two switching criteria](../figures/w05-switching.svg)
...
| $x_e^{\,p}$ | along-track error of §5-3 | m |
...
- **Neither of these is a mission-termination policy.** A real vehicle needs one — hold station at the last waypoint, loiter around it, or stop the propellers — and the choice belongs to the mission, not to the guidance law. Week 8 puts a Stateflow chart in charge of it.

## 4-7. Heading is not course — the debt from Week 4

- Week 4 §4-4 measured the crab angle and warned that a path-following law would eventually pay for it. This section is that bill, and it is the reason the two remaining laws of this week exist.
...
| $\beta$ | crab angle, over ground, from the Week 4 definition: $\operatorname{atan2}(v,u)$ | rad |
...
>
> The distinction matters because `beta_c` is a variable the learner sets in `W05_0_setup.m`, and it sets the current, never the crab angle. Nothing in the model lets a crab angle be commanded — it is an outcome.

### The cross-track error dynamics

- Before the offset can be derived, the rate of change of $y_e^{\,p}$ is needed, and it is used again in §5-8 and §5-9.
...
| **Sign** | $\chi > \pi_p$ turns the velocity towards starboard and increases $y_e^{\,p}$, consistent with §5-3 |
...
The autopilot has by then delivered its command, $\psi = \psi_d$, and $\psi_d$ is the LOS law of §5-4. Substituting both:
...
| **The third cost of a large $\Delta$** | the offset is *linear* in $\Delta$: doubling the look-ahead doubles the steady error. §5-5's trade has a third term after all |
...
- The remainder of this week is two different ways of removing this offset. §5-8 integrates it away without ever learning what caused it. §5-9 estimates the cause and cancels it directly.

### Three answers, in one picture

- Before either derivation, it is worth seeing what the two remaining laws are *for*. The figure below is the whole of §5-8 and §5-9 with no algebra in it.

![The idea behind ILOS and ALOS](../figures/w05-ilos-alos-idea.svg)
...
![Where each law adds its extra term](../figures/w05-three-laws.svg)
...
Where that number enters is the whole of §5-8 and §5-9:
...
| | Question | Where |
|---|---|---|
| 1 | **Why is a new law needed?** What plain LOS cannot do, measured | §5-8-1 |
| 2 | **What is the idea?** One sentence, before any algebra | §5-8-1a |
| 3 | **What is the law, and where does it come from?** | §5-8-2, §5-8-4 to §5-8-6 |
| 4 | **How is it applied?** Every parameter, with its unit and its source | §5-8-3, §5-8-8 |
| 5 | **What does it achieve, and what does it cost?** | §5-8-5a, §5-8-7 |

### 5-8-1. Why a new law is needed at all
...
- **The measurement.** Section 5-7 ran plain LOS in a $0.3$ m/s beam current. The vessel settled **$2.253$ m to one side of the path** and stayed there for the rest of the run. It did not oscillate and it did not drift further; it simply held station beside the line it was asked to follow.
...
### 5-8-1a. The idea, in one sentence
...
- **Where $K_p = 1/\Delta$ comes from.** It is not a new gain. The plain LOS law of §5-4 is $\psi_d = \pi_p - \arctan(y_e^{\,p}/\Delta)$, and writing $y_e^{\,p}/\Delta$ as $K_p y_e^{\,p}$ merely names the coefficient. The proportional gain of *every* LOS law in this week is $1/\Delta$, in units of $\text{m}^{-1}$, and §5-5's trade is therefore a statement about a proportional gain.
...
### 5-8-3. Every parameter, with its unit

| Symbol | Quantity | Unit | Where it comes from |
|---|---|---|---|
| $\Delta$ | look-ahead distance | m | §5-5; $8$ m, from the sweep of section E |
| $K_p$ | proportional gain, $1/\Delta$ | $\text{m}^{-1}$ | **not free** — fixed by $\Delta$ |
| $\kappa$ | integral gain constant | **m/s** | §5-8-7; $0.3$ m/s, from the sweep of section H |
...
> The course-angle sibling `ILOSchi.m` normalises differently — $\dot y_{int} = U y_e^{\,p} / \sqrt{\Delta^2 + a^2}$, which is $(\text{m/s})\cdot\text{m}/\text{m} = \text{m/s}$ — so there $y_{int}$ is a **length** and $\kappa$ is **dimensionless**. The same symbol $\kappa$ means two different physical quantities in two files of the same toolbox. A value of $\kappa$ carried from one to the other is not merely mistuned, it is dimensionally wrong. §5-8-6 explains why the two normalisations exist.

### 5-8-4. Why the denominator has that form — the anti-windup is in the law
...
- **This is the textbook definition of anti-windup**, and it arrives without a saturation block, without a clamp, and without a back-calculation gain. Week 3 §F built anti-windup by hand for the surge integrator, with a limiter and a feedback path; here the same effect is a property of the equation.
...
### 5-8-5. The equilibrium — where the offset goes
...
- Now the position condition. From §5-7, $\dot y_e^{\,p} = 0$ requires $\chi = \pi_p$, and $\chi = \psi_d + \beta$ once the autopilot has converged:
...
- Read the last line against §5-7. The quantity $\kappa y_{int}^{eq} = \Delta\tan\beta$ is **exactly the offset that plain LOS was left holding**. The integrator does not remove the crab angle and does not know it exists; it accumulates until the fictitious cross-track error it contributes equals the real one that the crab angle was producing, and from then on the real one can be zero.
...
### 5-8-5a. Watching the integrator fill

- The equilibrium above says *where* the integrator ends. This table says *how it gets there*, and it is the clearest way to see what the law is doing. It integrates the error dynamics of §5-8-6 directly, with no autopilot in the loop, so it is the idealised picture rather than the full model.
- Printed by `W05_G_current_and_integral.m`, at the end of its output.
...
|---|---|
| $y_e^{\,p}$ **grows first**, to $1.62$ m at $t = 10$ s | at $t = 0$ the integrator is empty, so the law is plain LOS and the current pushes the vessel off the path exactly as §5-7 says it must |
| $\kappa y_{int}$ climbs towards $2.255$ | the fourth column is the phantom error of the figure, filling up |
| bow tilt **overshoots** to $17.59°$ at $t = 25$ s | the integrator does not know when to stop; it overshoots and comes back, which is why §5-8-8's sweep has an interior minimum |
...
### 5-8-6. Stability — and why there are two normalisations

This is the longest derivation of the week, so it is worth knowing where it lands before starting.

| | | |
|---|---|---|
| 1 | **The current is eliminated, exactly.** Substituting the equilibrium of §5-8-5 makes $\beta$ cancel out of the error dynamics algebraically — not approximately | below |
...
- To argue stability, put the closed loop into error coordinates. Let $\tilde y = y_{int} - y_{int}^{eq}$ be the integrator's error, and assume for the argument that the autopilot is fast enough that $\psi = \psi_d$ and that $\beta$ is constant. Starting from §5-7's $\dot y_e^{\,p} = U\sin(\chi - \pi_p)$ with $\chi - \pi_p = \beta - \arctan(a/\Delta)$:
...
Substitute $a = y_e^{\,p} + \kappa y_{int} = y_e^{\,p} + \kappa\tilde y + \Delta\tan\beta$, using the equilibrium value from §5-8-5:
...
> The clean cancellation above is for the **course** form. The heading form of `ILOSpsi.m` differs from it only by the positive state-dependent factor $\Delta/(U D)$ multiplying the integrator, so it has the **same equilibrium** and the **same sign of integration** — §5-8-5 is untouched, and the numerical integration of §5-8-5 confirms convergence. What it does not have is this one-line quadratic proof. Børhaug, Pavlov and Pettersen (2008) prove the heading case by a cascade argument instead, and their result is uniform global asymptotic stability of the $(y_e^{\,p},y_{int})$ subsystem under a bound on $\kappa$.
...
### 5-8-7. The assumptions, and what breaks without them

| Assumption | Used where | What fails if it does not hold |
|---|---|---|
| $\psi = \psi_d$ — the autopilot is infinitely fast | $\chi = \psi_d + \beta$ in §5-8-5 and §5-8-6 | with real autopilot lag the true system is a cascade; $V$ is no longer monotone during transients. §5-9-7 measures exactly this |
...
> It has since been checked against the official one. `_tools/verify_alos.m` drives Fossen's `ALOSpsi.m` (MSS 2023+) and the equations of §5-9-2 side by side for 400 steps on this week's mission: **the largest disagreement in $\psi_d$ and in $y_e^{\,p}$ is exactly zero.** The law derived here is the published law, to the letter.
>
...
The same five questions as §5-8, in the same order.

| | Question | Where |
|---|---|---|
| 1 | **Why a second law, when ILOS already works?** | §5-9-1 |
| 2 | **What is the idea?** One sentence, before any algebra | §5-9-1a |
| 3 | **What is the law, and where does it come from?** | §5-9-2, §5-9-4 to §5-9-6 |
| 4 | **How is it applied?** Every parameter, with its unit and its source | §5-9-3, §5-9-8 |
| 5 | **What does it achieve, and what does it cost?** | §5-9-6a, §5-9-7, §5-9-9 |

### 5-9-1. Why a second law, when ILOS already works

Section 5-8 ends with the cross-track error at $0.008$ m. On the accuracy of this mission there is nothing left to improve, so a second law has to justify itself on something other than accuracy. It does, and the argument is about **what the controller knows**.
...
The contrast with §5-8 is exactly one word: ILOS adds its extra term **inside** the arctan, ALOS adds it **outside**. Everything that follows — the units, the stability argument, and the fact that one state is meaningful and the other is not — comes from that single structural choice, and §5-7's figure shows it side by side before any algebra.
...
- The guidance term is the plain LOS law of §5-4 with one extra term: the estimate of the crab angle, subtracted so that the *course* rather than the *heading* points where LOS wants it.
- The second line is written here as a differential equation and in `ALOSpsi.m` as its forward-Euler step. They are the same law; §5-10 puts the two side by side.
...
### 5-9-3. Every parameter, with its unit

| Symbol | Quantity | Unit | Where it comes from |
|---|---|---|---|
| $\Delta$ | look-ahead distance | m | §5-5; $8$ m |
| $\hat\beta$ | estimate of the crab angle | rad | a state of the guidance block |
| $\beta$ | the true crab angle, $\operatorname{atan2}(v,u)$ — the same $\beta$ as §5-7 and §5-8, and as Week 1 §1-12 and Week 4 §4-4 | rad | unknown to the law; measured only for verification |
| $\tilde\beta$ | estimation error $\beta - \hat\beta$ | rad | appears only in the analysis |
| $\gamma$ | adaptation gain | **rad/(m·s)** | §5-9-8; $0.005$, from the sweep of section H. `ALOSpsi.m` suggests $\gamma_h \approx 0.001$ and $\Delta_h = 5$–$20$ m as typical; the sweep of section H picks a faster gain for this hull and mission |
| $U$ | speed over ground, $\sqrt{u^2+v^2}$ | m/s | $0.774$ m/s in still water, which is exactly $X_{\text{ff}}/\lvert X_u\rvert = 60/77.55$. In a current it depends on the current's angle to the leg: `W05_guidance.slx` measures $0.956$ m/s averaged over the last $100$ s of section G's run. The derivation treats $U$ as constant; the §H script uses the logged value |
...
### 5-9-4. The error dynamics

- Start from §5-7, unchanged:
...
- **Read this before going on.** If the estimate were perfect, $\tilde\beta = 0$ and this is the plain LOS convergence of §5-4 with no offset at all. The whole problem has been reduced to driving one scalar, $\tilde\beta$, to zero.
...
### 5-9-5. The Lyapunov function, and why its weight is what it is
...
- Expand the sine of a difference. Writing $b = \arctan(y_e^{\,p}/\Delta)$, the right triangle of §5-4 gives
...
### 5-9-6. The adaptation law is forced, not chosen
...
| **Limit** $y_e^{\,p} \to \infty$ | the rate must not run away | $\dot{\hat\beta} \to \gamma\Delta$, bounded — a built-in rate limit, as in §5-8-4 |
...
### 5-9-6a. Watching the estimate converge

- The same idealised integration as §5-8-5a, run with the ALOS law instead, and printed by the same script. Compare the two tables column by column: they are doing the same job with different bookkeeping.
...
| What to notice | Why it happens |
|---|---|
| $y_e^{\,p}$ grows to $1.60$ m first | with $\hat\beta = 0$ the law *is* plain LOS, so the vessel drifts off exactly as in §5-7 |
...
| $y_e^{\,p} \to 0.000$ | once the estimate is right, $\tilde\beta = 0$ and §5-9-4 reduces to plain LOS with no disturbance left |

> [!tip] The one difference a beginner should take from the two tables
> ILOS's state ends at $7.516$ — **seven and a half what?** Seconds, as §5-8-3 shows, and the number means nothing on its own.
...
### 5-9-7. The assumptions, and the honest picture of $V(t)$

| Assumption | Used where | What fails if it does not hold |
|---|---|---|
| $\dot\beta = 0$ — constant current | $\dot{\tilde\beta} = -\dot{\hat\beta}$, §5-9-4 | a time-varying current leaves a term $\frac{U}{\gamma}\tilde\beta\dot\beta$ in $\dot V$ with no sign; the estimate lags and $V$ need not decrease |
| $\lvert\tilde\beta\rvert$ small | the linearisation of $\sin\tilde\beta$, §5-9-6 | the neglected cross term $U\Delta y_e(\sin\tilde\beta - \tilde\beta)/\sqrt{\Delta^2+y_e^2}$ makes $\dot V$ positive in a thin band near the path; the stability class that survives this is Fossen's (2023) result, not something §5-9-6 proves |
...
| anti-windup | built into the denominator, §5-8-4 | built into the bounded $\Delta y_e^{\,p}/\sqrt{\Delta^2+y_e^2}$ |
...
- Every line of the guidance block is generated by `guidance_code(law)` inside `W05_1_build_guidance.m`, so the four laws share every line except the body. The tables below put each line beside the equation it implements.
...
| stop the index at $n-1$, §5-6 | `if hit && k < n-1, k = k + 1; end` |
...
|---|---|---|---|---|
| **Curved paths (Hermite spline)** | replace the straight leg by a monotone cubic spline; $\pi_p$ becomes the spline's tangent angle at the closest point | continuous curvature, so no corner transient at all — the switching logic of §5-6 disappears | a closest-point search each step, and a curvature term in the error dynamics | Lekkas and Fossen (2014), *IEEE TCST* 22(6), 2287–2301 |
...
| **Model predictive path following** | optimise the future track over a horizon subject to the actuator limits of Week 6 | handles input saturation and obstacles *inside* the guidance layer | an optimisation every step; no closed-form law to verify by hand | Faulwasser and Findeisen (2016), *IEEE TAC* 61(4), 1026–1039 |
| **ALOS in 3-D** | estimate both the sideslip and the angle of attack, for a vehicle that can dive | underwater path following with the same structure as §5-9 | two adaptive states, and a coupled stability argument | Fossen and Aguiar (2024), *Ocean Engineering* |
...
> Obstacle avoidance, collision regulations, and any form of replanning. Those change the *waypoint list*, which is a layer above guidance, not a change to the guidance law. Week 8 introduces the state machine that owns that layer.
...
- Every script is in `lectures/W05_simulink/`. All of them drive the same model, `W05_guidance.slx`, and all of them read it back with the same `W05_read.m`.
- The shared files are `W05_vars.m` (every number), `W05_read.m` (unpacks the log), `W05_plot.m` (the standard track figure) and `W05_1_build_guidance.m` (builds the model from nothing).
...
`W05_0_setup.m` is **the only file a student edits.** It copies every number out of `W05_vars.m` into the base workspace, so the model can be opened and run by hand.
...
|---|---|---|---|
| 0 | `W05_0_setup.m` | A | the base workspace. **The only file to edit this week** |
| 1 | `W05_1_build_guidance.m` | B | `W05_guidance.slx` and its block diagram |
| C | `W05_C_aim_at_the_waypoint.m` | C | `img/W05_result_atan2.png` |
| D | `W05_D_line_of_sight.m` | D | `img/W05_result_los.png` |
| E | `W05_E_lookahead_distance.m` | E | `img/W05_result_lookahead.png` |
| F | `W05_F_waypoint_switching.m` | F | `img/W05_result_switching.png` |
| G | `W05_G_current_and_integral.m` | G | `img/W05_result_current.png` |
| H | `W05_H_adaptive_and_stability.m` | H | `img/W05_result_stability.png` |
...
- The remaining files — `W05_vars.m`, `W05_read.m`, `W05_plot.m`, `W05_animate.m` — are called **by** the section scripts and by the model. They are never run by hand.
- The laboratory of the second hour lives in `W05_simulink/problems/` and `solutions/`, and is separate from these.
...
| `feasibility ... yes` | the §5-6 condition $R < \min_k d_k$. If this ever prints `NO`, stop: a waypoint will be skipped |
| `Delta / L 4.0` | the rule of thumb of §5-5, printed as a sanity check on a value that section E actually justifies |
| `Ki = 0.0375` | $\kappa/\Delta = 0.3/8$, the derived gain of §5-8-2 — not an independent number |
...
> ```
> This writes `W05_guidance.slx` and `img/W05_guidance.png`. Every other section calls it automatically if the model is missing, so it need only be run by hand after the model has been broken.

![The W05 model](W05_simulink/img/W05_guidance.png)
...
| blue, **Autopilot bank** | four identical copies of the Week 4 P–D law. Nothing here is retuned between rows |
...
- **The backward line is the whole point of the week.** Weeks 1 to 4 had a command that came from outside the model. Guidance is feedback: it cannot say where to point until it is told where the vessel is.
...
- Set `guid_show = 1..4` in `W05_0_setup.m` to plot a single row instead of all four.

> [!tip] Checking the model is still sound
> ```matlab
> check_overlaps('W05_guidance')
> ```
...
> ```
> Produces `img/W05_result_atan2.png`.
...
![atan2 against LOS](W05_simulink/img/W05_result_atan2.png)
...
LOS does nothing at all at the same moment, because its aim point sits $\Delta$ ahead on the *extension* of the last leg and therefore stays ahead of the vessel for ever. Neither behaviour is a mission-termination policy, as §5-6 notes; this figure is what the absence of one looks like.
...
> W05_D_line_of_sight
> ```
> Produces `img/W05_result_los.png`.

Expected output:

```
  W05 section D — psi_d = pi_p - atan(y_e / Delta)
...
|---|---|
| `settled |y_e|` | mean $\lvert y_e^{\,p}\rvert$ over the **last quarter** of the run, $375$ to $500$ s — the same window `W05_plot.m` prints on every track figure |
...
- The first number is a check on the **logging**, not on the law: $\psi_d$ is recomputed here from the logged $y_e^{\,p}$ using the equation of §5-4, and compared with the logged $\psi_d$. Agreement to $0.000\text{e}{+}00$ confirms that the signal named `y_e` in the log really is the one the block used.

![The LOS law taken apart](W05_simulink/img/W05_result_los.png)
...
> cd lectures/W05_simulink
> W05_0_setup
> W05_E_lookahead_distance
> ```
> Produces `img/W05_result_lookahead.png`. Five runs, one per value of $\Delta$.
...
![The look-ahead trade](W05_simulink/img/W05_result_lookahead.png)
...
> ```
> Produces `img/W05_result_switching.png`. Four runs, one per value of $R$.
...
- The first block is §5-6 as arithmetic. One position, two tests, two different answers: the along-track remainder is $4.50$ m and passes; the true distance is $10.97$ m and fails by more than a factor of two.
...
![Switching radius](W05_simulink/img/W05_result_switching.png)
...
| left, horizontal dotted lines | the **switching thresholds**. They are lines and not circles because the test is $d_k - x_e^{\,p} < R$ along the leg — §5-6 |
...
Notice first that the thresholds are **lines, not circles**. Leg 1 runs due North along $E = 0$, so the along-track test $d_k - x_e^{\,p} < R$ is the half-plane $N > 60 - R$, and its boundary is horizontal. That is the distinction §5-6 draws, shown here on the data — circles would have contradicted the page before.
...
> cd lectures/W05_simulink
> W05_0_setup
> W05_G_current_and_integral
> ```
> Produces `img/W05_result_current.png`. One four-vessel run, a six-point sweep of the current speed, and finally the two state-filling tables printed in §5-8-5a and §5-9-6a.

Expected output:

```
  W05 section G — a current of 0.30 m/s at 90 deg
...
|---|---|
| `LOS 2.253` | the §5-7 prediction $y_{e,ss}^{\,p} = \Delta\tan\beta$, to $0.0012$ m |
| `ILOS 0.008` with `aux 7.519` | the §5-8-5 equilibrium: $y_{int}^{eq} = \Delta\tan\beta/\kappa = 7.5158$ s predicted, $7.519$ s measured |
...
![Current, ILOS and ALOS](W05_simulink/img/W05_result_current.png)
...
| middle, dashed grey | $\Delta\tan\beta = 2.25$ m — **predicted in §5-7 before the run**, not fitted to it |
...
The blue LOS curve in the middle panel is the most important line in the week. It settles onto the dashed grey line — and that grey line was **not fitted to the data.** It is $\Delta\tan\beta = 2.25$ m, drawn from §5-7 before the run was made.
...
> cd lectures/W05_simulink
> W05_0_setup
> W05_H_adaptive_and_stability
> ```
> Produces `img/W05_result_stability.png`. Ten runs — five values of $\kappa$, five of $\gamma$ — plus the Lyapunov computation on leg 1. The $\kappa$ sweep is reported as the table below rather than as a third panel; the figure keeps only what the section is about.
...
![Gains and the Lyapunov function](W05_simulink/img/W05_result_stability.png)
...
| right | $V = \tfrac12 y_e^2 + \tfrac{U}{2\gamma}\tilde\beta^2$, the Lyapunov function of §5-9-5, over leg 1 only |
| right, open marker | the peak of $V$, at $t = 17.5$ s |

**What the figure says**

The left panel is what each $\gamma$ believes the crab angle to be, against the truth. The right panel is the Lyapunov function of §5-9-5, evaluated on the actual vessel over leg 1.
...
That rise is not a contradiction of §5-9-6, and it is worth being precise about why. The small-angle model there gives $\dot V \le 0$ *on the assumption that $\psi = \psi_d$ exactly*. Between the guidance law and the water sits a Week 4 autopilot with its own settling time, and while the heading is still catching up the error dynamics the proof describes are not yet the dynamics the vessel has.

This is reported rather than smoothed away deliberately. A monotone $V$ was available: integrate the kinematic subsystem on its own, from the same start and with the same current, and $V$ falls at every one of $50\,001$ samples (`_tools/verify_review_math.m`). (The band of §5-9-6 where $\dot V > 0$ exists, but that trajectory never enters it. The first time $y_e^{\,p}$ turns negative, at $t = 80.2$ s, $\tilde\beta$ has already changed sign; later, in the convergence tail, the two do disagree in sign again, but by then $\tilde\beta$ is so small that the band is under $10^{-11}$ m wide.) Plotting that would have been a picture of an assumption rather than of a vessel. What the reference actually supports is that the guidance subsystem is USGES and that its cascade with a stable autopilot is stable. It does **not** claim that $V$ of the outer loop falls at every instant of a real run, and this figure is what the difference looks like.
...
|---|---|---|
| §5-2, §5-3 | derived $\pi_p$ and the pair $(x_e^{\,p}, y_e^{\,p})$ from one rotation | compared against MSS `crosstrackWpt.m` at eight test points; largest disagreement $0$ |
| §5-4 | derived the LOS law from the aim-point geometry | five limits checked numerically; compared against MSS `LOSchi.m`; recomputed from the log in section D, agreement $0.0\text{e}{+}00$ deg |
| §5-5 | fixed $\Delta = 8$ m | five-point sweep, section E: settling distance and overshoot move in opposite directions and $8$ m is where the second stops improving |
| §5-6 | established the two switching criteria and $R < \min_k d_k$ | section F evaluates both tests on one position — $4.50$ m passes, $10.97$ m fails; four-point sweep of $R$ |
| §5-7 | derived $\dot y_e^{\,p} = U\sin(\chi-\pi_p)$ and $y_{e,ss}^{\,p} = \Delta\tan\beta$ | six-point current sweep, section G; worst disagreement $0.002$ m on a $4.1$ m offset |
| §5-8 | derived ILOS, its units, its built-in anti-windup and its equilibrium | $y_{int}^{eq} = \Delta\tan\beta/\kappa$ predicted $7.5158$ s, measured $7.519$ s; closed form of $\dot y_e^{\,p}$ checked to $6.7\times10^{-16}$ over $10^4$ states |
| §5-8-6 | showed the course normalisation admits a constant-weight Lyapunov function and the heading one does not | $\dot V$ matches $-U\cos\beta y_e^2/D$ to $1.1\times10^{-14}$; the heading form leaves a residual of rms $2.94$ |
| §5-9 | derived ALOS and showed the adaptation law is forced | $\hat\beta = 15.91°$ against a true $15.88°$, with nothing in the law told what the current was; and the derived law checked against Fossen's `ALOSpsi.m` over 400 steps — **disagreement exactly zero** |
| §5-10 | put every equation beside the line of MATLAB that implements it | the code shown is generated by `guidance_code(law)`, so it cannot drift from the model |
...
- [ ] `W05_0_setup` prints `feasibility ... yes` and `Ki = 0.0375`.
- [ ] `check_overlaps('W05_guidance')` returns **0**.
...
The second hour of the Week 5 session is spent building the guidance block in Simulink. A complete Week 4 vessel is provided with one input left dangling: the commanded heading.
...
- The full problem sheet is [`W05_simulink/problems/README.md`](W05_simulink/problems/README.md), and reference answers are in [`W05_simulink/solutions/`](W05_simulink/solutions/README.md).
- The problem sheet carries **the result graphs a correct model produces**.
- **Nothing inside the Week 4 autopilot changes.** That is the practical content of "guidance and control are separate layers", and the laboratory model makes the seam visible.
- Problem 3 ends where §5-8 begins: a vessel holding its commanded heading perfectly and still $3.3$ m from where it was asked to be.
...
| Symptom | Cause | Fix |
|---|---|---|
| `Invalid setting for parameter 'Value' ... 'mp'` when building | Constant blocks hold variable **names**, which Simulink validates at creation, and the base workspace was empty | `ensure_base_vars(W05_vars)` at the top of the builder |
...
| `SampleTime 'h'` suddenly invalid, with no edit to the model | an ad-hoc diagnostic in the base workspace had overwritten `h` with a port-handle struct | never reuse a model variable's name in a scratch command; re-run `W05_0_setup` |
| A section's figure looks identical to the previous section's | `W05_plot` was called for both | it is a defect, not a style choice — section D now plots the decomposition of the law instead |
...
| Børhaug, E., Pavlov, A. and Pettersen, K. Y. (2008). Integral LOS control for path following of underactuated marine surface vessels in the presence of constant ocean currents | *Proc. 47th IEEE CDC*, 4984–4991. The ILOS law of §5-8 |
...
| Nelson, D. R. et al. (2007). Vector field path following for miniature air vehicles | *IEEE Trans. Robotics* 23(3), 519–529. §5-11 |
| Faulwasser, T. and Findeisen, R. (2016). Nonlinear model predictive control for constrained output path following | *IEEE TAC* 61(4), 1026–1039. §5-11 |
| **Fossen, T. I. (2023). An Adaptive Line-of-sight (ALOS) Guidance Law for Path Following of Aircraft and Marine Craft** | *IEEE TCST* **31**(6), 2887–2894, [doi:10.1109/TCST.2023.3259819](https://doi.org/10.1109/TCST.2023.3259819). The ALOS law of §5-9 and its USGES proof |
| MSS `ILOSpsi.m`, `ILOSchi.m`, `LOSchi.m`, `crosstrackWpt.m` | the vendored 2021 release, located by `_tools/mss_path.m` |
| MSS `ALOSpsi.m` (2023+) | not in the vendored release; `_tools/verify_alos.m` locates a newer copy on this machine and checks §5-9 against it |

> [!note] A correction worth recording
> An earlier draft of this week stated that ALOS had **no reference implementation to check against**, because the vendored MSS is the 2021 release. That was wrong: `ALOSpsi.m` exists in MSS 2023 and later, and a copy is present on this machine. The derivation of §5-9 was written before that copy was found and turned out to match it **exactly** — `_tools/verify_alos.m` reports zero disagreement in both $\psi_d$ and $y_e^{\,p}$ over 400 steps.
...
**Week 6 — Control Allocation.** This week gave all four vessels the same square allocation from Appendix A1 and never questioned it. Week 6 asks what happens when there are more actuators than degrees of freedom, when a thruster saturates, and when one fails outright — and turns $\boldsymbol{\tau} = \mathbf{B}\mathbf{f}$ from a matrix inverse into an optimisation.
