...
| Toolboxes | none beyond Simulink for this week; the optional Experiment 1-10c also needs Simulink 3D Animation and a transmitter with a USB joystick mode |
...
Every section states what is being described, derives it, and then runs **the experiment that measures it**. The short ones are two lines in the Command Window; the long ones are the Simulink models of this week. Nothing in the text is quoted from anywhere but a run.

| Part of a section | What it holds |
|---|---|
| the text | why the quantity is needed, the derivation, and the source it is checked against |
| **Experiment N-x** | the commands, the output actually obtained, and — where there is a figure — a table reading it back to the derivation |

Sections 1-1 to 1-8 build the description of motion, and their experiments are short calculations that check it. Sections 1-9 to 1-13 run the Otter itself, and their experiments are the laboratory of this week.
...
## 1-0. Setting up and running (15 min)
...
|---|---|---|---|
| 0 | `W01_0_setup.m` | 1-0 | the base workspace, so the model can be run from Simulink |
| 1 | `W01_1_build_openloop.m` | 1-9 | `W01_openloop.slx` and `img/W01_openloop.png` |
| 2 | `W01_C_terminal_speed.m` | 1-11 | `img/W01_result_speed.png` |
| 3 | `W01_D_the_manoeuvre.m` | 1-12 | `img/W01_result_track.png` |
| 4 | `W01_E_build_current.m` | 1-13 | `W01_current.slx` and `img/W01_current.png` |
| 5 | `W01_E_current_run.m` | 1-13 | `img/W01_result_current.png` |
| 6 | `W01_F_build_interactive.m` | 1-10a | `W01_interactive.slx` and `img/W01_interactive.png` |
| 7 | `W01_F_button_check.m` | 1-10a | the table of Experiment 1-10a — one headless run per button |
| 8 | `W01_G_build_rc.m`, `W01_G_rc_check.m` | 1-10b | `W01_rc.slx`, `img/W01_rc.png` and the table of Experiment 1-10b |
| 9 | `W01_H_build_rc_usb.m`, `W01_H_usb_check.m` | 1-10c | `W01_rc_usb.slx` and its table (optional hardware) |
| — | `W01_frames.m` | 1-4 | the table of §1-4; draws a figure and saves nothing |

- **Each experiment is one script and one model.** Running it leaves exactly the numbers and the figures its section discusses, so a class can work through the week a page at a time.
- Experiments 1-11, 1-12 and 1-13 build the model they need if it is missing, so any one of them can be run first.
...
- `W01_frames.m` is the script of Experiment 1-4: it prints the worked example of §1-4 and the four panels of its figure, and saves nothing. The same applies to the three-panel and drift-rose drawings inside `W01_cur_plot.m` — that file is still the current model's `StopFcn`, but Experiment 1-13 no longer calls those two drawings.
...
### Experiment 1-3 · The three properties, on a real matrix (5 min)

**What it measures.** The three properties listed above, on one rotation matrix: that its inverse is its transpose, that its determinant is $+1$, and that it leaves lengths unchanged. No model and no simulation — the matrix is built by the MSS function every later week uses.
...
| Printed | Which property it checks | Why it matters later |
|---|---|---|
| $\det \mathbf{R} = 1$ | a rotation, not a reflection | a frame drawn with $-1$ would have its axes mirrored, and every sign in the course would flip |
| $\lvert \mathbf{R}^\top\mathbf{R} - \mathbf{I}\rvert \approx 10^{-16}$ | orthogonality | the inverse is free: going from NED back to the body needs no matrix inversion, only a transpose |
| $\lvert \mathbf{R}^{-1} - \mathbf{R}^\top\rvert \approx 10^{-16}$ | the same property, stated as MATLAB would use it | `R'` is what the models write, and it is exact rather than merely accurate |
| $\lvert \mathbf{v}\rvert = \lvert \mathbf{R}\mathbf{v}\rvert$ | length is preserved | a speed is the same speed in either frame; only its components change (§1-4) |

| What to try | What to watch |
|---|---|
| `R2 = Rzyx(0,0,deg2rad(30)); R2(1:2,1:2)` | the planar matrix above, the only one Weeks 3 to 5 use |
| `det(Rzyx(1,2,3))` | still $1$: the properties hold at every attitude, not only at small angles |
...
### Experiment 1-4 · One velocity, read in two frames (5 min)

**What it measures.** The worked example above and the four panels of the figure, computed rather than quoted: the two frames, the length check, and the crab angle.
...
Expected output (the second table is the four panels):
...
**Reading the output against the section.**

| Where to look | What is there | What it shows |
|---|---|---|
| the $u$ column, panels 1 to 3 | $1.00$ every time | $u$ is measured along the hull, and the hull turning does not change it |
| the $\dot x^n$ column, panels 1 to 3 | $1.00$, $0.00$, $0.71$ | the same motion, read against axes that do not turn |
| the $\lvert V\rvert$ column | $1.0000$ three times | a rotation preserves length — the cheapest check on any frame conversion |
| panel 4 against panel 1 | same $\psi$, different $\dot y^n$ | sway, not heading, is what makes the vessel travel off its bow direction |
| the $\beta$ and $\chi$ columns, panel 4 | $26.565°$, equal to each other at $\psi = 0$ | the crab angle: where the vessel goes is not where it points |

| What to try | What to watch |
|---|---|
| open `W01_frames.m` and change panel 4 to `v = -0.5` | $\beta$ becomes $-26.565°$: sway to port carries the vessel to port of its bow |
| compare with the worked example printed first | $\psi = 30°$, $u = 2$, $v = 0.5$ gives $\beta = 14.036°$ and $\chi = 44.036°$, and `atan2(yn_dot, xn_dot)` returns the same $44.036°$ |
...
### Experiment 1-5 · The singularity, and the quaternion that does not have it (5 min)

**What it measures.** The two claims of this section: that $\mathbf{T}_\Theta$ blows up as the pitch approaches $90°$ — $\mathbf{R}$ never does — and that a quaternion carries the same attitude through it, returning the original angles exactly.
...
| Where to look | What is there | What it shows |
|---|---|---|
| the first column of numbers | $1.00$, $1.39$, $5.67$, $56.43$, $564.25$ | $\tan\theta$ in the matrix: every $10\times$ nearer to $90°$ multiplies the entries by about $10$ |
| the same column at $\theta = 0$ | exactly $1.00$ | level, $\mathbf{T}_\Theta$ is the identity and the Euler rates are the gyro rates |
| the round trip | $10$, $20$, $30$ back to the digit | a quaternion loses nothing; it simply has no singular attitude to lose it at |
| $\lvert \mathbf{q}\rvert = 1$ | exactly one | the constraint that replaces the third Euler angle's freedom |

- The Otter never pitches steeply, so this course uses Euler angles throughout. A quadrotor or an AUV that can point straight up cannot, and §1-5 is where that choice is made rather than discovered.

| What to try | What to watch |
|---|---|
| `Tzyx(deg2rad(10), deg2rad(90))` | MATLAB returns `Inf`: the matrix does not exist at the singularity, and no amount of numerical care recovers it |
| `max(max(abs(Rzyx(deg2rad(10), deg2rad(90), 0))))` | $1$ — the **rotation** matrix is perfectly well behaved at the same attitude. It is the rate conversion that fails, not the attitude |
...
### Experiment 1-7 · Which three rows survive (5 min)

**What it measures.** Condition 1 of the callout above, on the vessel itself: with both propellers driven equally, `otter.m` is asked for $\dot{\boldsymbol{\nu}}$ and the six rows are read. Nothing is assumed — the discarded degrees of freedom are measured.

**Opening and running.** No model and no simulation: one call to the plant function, at a state chosen by hand.
...
**Reading the output against the three conditions.**

| Row | Value | What it says |
|---|---|---|
| $\dot u$ | $-0.1732$ | surge is alive: at 2 m/s the drag exceeds the thrust of 80 rad/s, so the vessel is still slowing towards its terminal speed (§1-11) |
| $\dot v$, $\dot r$ | exactly $0$ | with equal propellers nothing drives sway or yaw — **condition 1**, no actuation, and no asymmetry to start them |
| $\dot w$, $\dot q$ | $-0.0070$, $-0.0771$ | not zero, but one to two orders below $\dot u$: the vessel is settling into its trim, pulled back by buoyancy — **condition 2** |
| $\dot p$ | $0.0000$ | a catamaran driven symmetrically has no reason to roll |

- The discarded rows are **small, not absent**, and `otter.m` keeps all six so that the size can be read rather than assumed.

| What to try | What to watch |
|---|---|
| `otter(x, [90; 60], mp, rp, 0, 0)` with `x(6) = 0.3` | turning: $\dot v = -0.2339$ and $\dot r = -0.9618$ — sway and yaw wake up together, which is the coupling §1-12 explains |
| `otter(x, [80; 80], 0, rp, 0, 0)` — the payload removed | the pitch row barely moves, $-0.0711$ against $-0.0771$: most of that trim belongs to the hull being driven, not to the 25 kg on board |
...
### Experiment 1-8 · The last line, checked against §1-4 (5 min)

**What it measures.** The kinematic half of the last line, $\dot{\boldsymbol{\eta}} = \mathbf{J}(\boldsymbol{\eta})\boldsymbol{\nu}$: that the position rates `otter.m` returns are the rotation matrix of §1-3 applied to the body velocity, and that they are the very numbers §1-4 worked by hand.

**Opening and running.** One call to the plant, at the state of the §1-4 example.
...
| Where to look | What is there | What it shows |
|---|---|---|
| the two rows | identical to every digit, difference $0.00\times10^{0}$ | the last line of `otter.m` is exactly $\mathbf{J}(\boldsymbol{\eta})\boldsymbol{\nu}$, with no hidden step |
| the same two numbers in §1-4 | $1.4821$ and $1.4330$ m/s | the worked example was not an illustration: it is what the plant computes |
| that they came from $u = 2.0$, $v = 0.5$ | neither rate equals either velocity | §1-4's whole point, now inside the plant |

- The other half of the line, $\dot{\boldsymbol{\nu}} = \mathbf{M}^{-1}(\cdot)$, is what Experiment 1-7 read row by row. `verify_w01_theory` rebuilds $\mathbf{M}$ from the same lines of `otter.m` and checks the Coriolis coefficients that §1-12 uses.

| What to try | What to watch |
|---|---|
| `x(12) = 0;` and run again | the rates become $2.0$ and $0.5$: pointing north, body and NED components coincide, which is the one case in which integrating $u$ would have been right |
| `x(12) = deg2rad(90);` | $-0.5$ and $2.0$: the surge velocity now shows up entirely as an **east** rate |
...
### Experiment 1-9 · Reading the model (15 min)
...
- Those are not the same question. A marine vehicle carries a sway velocity, so its heading $\psi$ and its course over ground differ by the crab angle $\beta = \operatorname{atan2}(v, u)$. In the turns of Experiment 1-12 they differ by $7.05°$, and in the beam current of Experiment 1-13 by $25.8°$; no track drawn on its own can show either. Week 5 has to steer around exactly this difference.
...
### Experiment 1-10a · Driving the vessel by hand (15 min)

- Experiments 1-11 to 1-13 each ran a command fixed in advance. This section hands the command over: one model, five buttons, two sliders, and the same live view as Experiment 1-9.
...
- Each button applies one rule — the same rule as the timetable of Experiment 1-12:
...
- PORT and STARBOARD are mirror images. With $dn = 10$ rather than the $3.5$ of Experiment 1-12, the turn is about twice as fast and the crab angle $\beta$ grows from $7.05°$ to $10.11°$.
...
| PORT, then STARBOARD | $v$ changes sign; the bow points off the track by $\beta$ | §1-12 and Experiment 1-12 |
...
### Experiment 1-10b · An RC transmitter, and the allocation behind it (25 min)

- In Experiment 1-10a the operator commanded shaft speeds. To drive straight at a chosen speed, the operator had to know the propeller curve $T = k\,n\lvert n\rvert$; to turn, the operator had to know that the yaw moment is $N = y_{\text{pont}}(T_L - T_R)$. A real operator — a person on a transmitter, the joystick of a dynamic-positioning console, or the autopilot of later weeks — asks instead for a **force and a moment**. Turning that request into shaft speeds is **control allocation**.
...
| | Experiment 1-10a `W01_interactive.slx` | Experiment 1-10b `W01_rc.slx` |
|---|---|---|
...
| `Otter USV`, `Measurements` | the plant and measurement stage of Experiment 1-9. The log holds $[u\ v\ r\ N\ E\ \psi]$, then $\mathbf{n}$, $\boldsymbol{\tau}_d$ and $\boldsymbol{\tau}_a$ |
...
- The live view, drawn by `W01rc_animate.m` on the right half of the screen, adds one panel to the view of Experiment 1-10a: the attainable set, with the demanded force as an open circle and the delivered force as an orange dot. It is drawn in stick directions — surge force up, starboard moment to the right — so pushing the throttle forward moves the circle up. Below it are $u$ with the prediction $X_a/\lvert X_u\rvert$, $r$, and $n_L$, $n_R$ with the saturation limits.
...
- As in Experiment 1-10a, a person on the sticks never repeats a run, so the numbers come from a script. `W01_G_rc_check.m` holds each stick position for $60$ s from rest, in Mode 2, with pacing and the live view switched off.
...
> In every straight-line case the settled speed equals $X_a/\lvert X_u\rvert$ to four decimals — $1.5432$, $3.0864$ and $-1.7203$ m/s — so the speed is **proportional to the throttle**: half throttle, half speed. In Experiment 1-10a, halving the SPEED slider gave a quarter of the speed, because the command was $n$ and thrust grows with $n\lvert n\rvert$. Allocation inverts that curve, so whatever sits upstream — a thumb on a stick, or a speed controller — acts on a plant that is linear in its command.
...
- Over USB a transmitter sends its **channels** — aileron, elevator, throttle, rudder — each as one axis from $-1$ to $+1$. The mode of Experiment 1-10b is applied inside the transmitter before anything leaves it. The throttle therefore arrives on the same axis whichever hand moves it, and `W01_rc_usb.slx` has no MODE switch.

| | Experiment 1-10b `W01_rc.slx` | Experiment 1-10c `W01_rc_usb.slx` |
|---|---|---|
...
- The block `USB receiver` turns the two chosen axes into the stick deflections of Experiment 1-10b, through the same neutral zone:
...
| $d_b$ | neutral zone, as in Experiment 1-10b | $3$ |
...
| the rest of the chain | identical to the model of Experiment 1-10b |
...
- Half ahead with half left rudder reproduces case 7 of Experiment 1-10b to every printed digit, whatever the channel order and the direction of the throttle axis, once the model is told which is which.
...
- Cross-flow drag is the only term that opposes the drift, so $v$ grows until the drag it generates equals $M_{11}ur$. Measured by `_tools/verify_w01_theory.m` in the steady port turn of Experiment 1-12, $n = [56.5,\ 63.5]$ rad/s:
...
### Experiment 1-12 · One manoeuvre: straight, port, straight, starboard, straight (30 min)
...
- That single asymmetry produces every result of Experiment 1-13: the hull settles at the same speed *through the water* in all four runs, and ends up in four different places.
...
### Experiment 1-13 · The same command in four currents (25 min)
...
|---|---|---|---|
| 1-0 | — | `W01_0_setup` | the parameters every block reads |
| 1-3 | none — the Command Window | — | the three properties of a rotation matrix |
| 1-4 | none — the Command Window | `W01_frames` | one velocity, read in two frames |
| 1-5 | none — the Command Window | — | the singularity of $\mathbf{T}_\Theta$, and the quaternion round trip |
| 1-7 | none — one call to `otter.m` | — | which three rows of $\dot{\boldsymbol{\nu}}$ survive |
| 1-8 | none — one call to `otter.m` | — | the last line checked against §1-4 |
| 1-9 | `W01_openloop` | `W01_1_build_openloop` | the model read block by block |
| 1-10a | `W01_interactive` | `W01_F_build_interactive`, `W01_F_button_check` | the vessel driven by buttons |
| 1-10b | `W01_rc` | `W01_G_build_rc`, `W01_G_rc_check` | an RC transmitter and the allocation behind it |
| 1-10c | `W01_rc_usb` | `W01_H_build_rc_usb`, `W01_H_usb_check` | a real transmitter over USB (optional hardware) |
| 1-11 | `W01_openloop` | `W01_C_terminal_speed` | the terminal speed predicted, then measured |
| 1-12 | `W01_openloop` | `W01_D_the_manoeuvre` | one manoeuvre, and the sway it produces |
| 1-13 | `W01_current` | `W01_E_build_current`, `W01_E_current_run` | the same command in four currents |

- Experiments 1-3 to 1-8 are short calculations: a few lines in the Command Window, no model, no figure saved.
- Experiments 1-11 and 1-12 share `W01_openloop` because they ask two questions of **one** open-loop run — the speed along the leg, and the sway in the turn. Every other experiment of the course has a model to itself.
...
| 9 | Replaced the on-screen transmitter by a USB transmitter | a fake transmitter reproduces case 7 of Experiment 1-10b exactly for AETR, TAER and an inverted throttle axis; the physical device is checked on first connection |
...
- Fossen, T. I. *Handbook of Marine Craft Hydrodynamics and Motion Control*, 2nd ed. Chapters 2 (kinematics), 3 (rigid-body dynamics) and 6 (manoeuvring models, including the cross-flow drag model). §2.2.2 unit quaternions, §2.2.3 quaternions from Euler angles, §2.2.4 Euler angles from quaternions, §11.2 control allocation (Experiment 1-10b). The section numbers were checked against the 1st edition and the 2020 manuscript of the 2nd edition, because the printed 2nd edition available to this course is a scan without searchable text.
...
- `W01_simulink/W01_F_build_interactive.m` · `W01_interactive.slx` · `W01i_animate.m` · `W01i_control.m` · `W01_F_button_check.m` — the model driven by buttons, its live view with a following window and a shaft-speed panel, the handler of its START and STOP buttons, and the headless check behind the table of Experiment 1-10a
- `W01_simulink/W01_G_build_rc.m` · `W01_rc.slx` · `W01rc_animate.m` · `W01rc_control.m` · `W01_G_rc_check.m` — the model driven by an RC transmitter through a control allocation, its live view with the attainable set, the handler of its START, STOP and CENTRE buttons, and the headless check behind the table and figure of Experiment 1-10b
- `W01_simulink/W01_H_build_rc_usb.m` · `W01_rc_usb.slx` · `W01_H_usb_setup.m` · `W01rc_usb_axis.m` · `W01_H_usb_check.m` — the same chain read from a USB transmitter, its calibration, the axis finder the calibration uses, and the check with a fake transmitter behind the table of Experiment 1-10c
- `_tools/hmi_bind.m`, `_tools/image_button.m` — a Dashboard block bound to a Constant, and a clickable button image on the canvas, used by the models of Experiment 1-10a and Experiment 1-10b
