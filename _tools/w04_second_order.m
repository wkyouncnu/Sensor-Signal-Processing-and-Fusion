function w04_second_order()
%W04_SECOND_ORDER  W04 §4-3 「2차 표준형이 zeta 와 wn 에 따라 어떻게 바뀌는가」.
%
%      w04_second_order
%
%  figures/src/w04-second-order.tex 를 낸다.
%
%  왜 시간영역 두 판인가.
%    사용자가 W03 의 극-영점 지도를 보고 **"너무 어려워"** 라고 했다 (2026-09-08).
...
%    영점이 없는 표준형이므로 이 공식이 정확히 맞아야 한다 — W03 와 다른 점이고,
%    그 대조가 여기서 공식을 쓸 자격을 준다.

here = fileparts(mfilename('fullpath'));
root = fileparts(here);
out  = fullfile(root, 'figures', 'src', 'w04-second-order.tex');

%% ---- 이 강의의 설계점 --------------------------------------------------
WN_D = 1.5312;      % rad/s  — W04 이 고른 값
ZE_D = 0.9;
...
W('%% figures/w04-second-order.svg — zeta 는 모양을, wn 은 속도를 정한다.\n');
W('%%\n');
W('%%   bash _tools/tikz2svg.sh figures/src/w04-second-order.tex figures/w04-second-order.svg\n');
W('%%\n');
W('%% 손으로 쓰지 않는다 — _tools/w04_second_order.m 이 계산해서 낸다.\n');
