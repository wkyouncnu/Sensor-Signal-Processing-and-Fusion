function ok = verify_w03_antiwindup()
%VERIFY_W03_ANTIWINDUP  안티와인드업 세 방식의 결과가 옳은지 두 가지로 확인한다.
%                       Check the results of the three anti-windup schemes two
%                       independent ways.
%
%      ok = verify_w03_antiwindup
%
%VERIFY_W03_ANTIWINDUP  안티와인드업 세 방식의 결과가 옳은지 여러 방향에서 본다.
%
...
fprintf('\n  W03 §3-6 — 안티와인드업 검증 / anti-windup verification\n\n');
...
R = cell(1,3);
for i = 1:size(REF,1)
    R{i} = run_sim('W03_surge_control', V, 'aw_mode', REF{i,2});
    pI = max(R{i}.y(:, c.I));
...
%     제어기   X_cmd = Kp e + I,            e = u_d - u          (§3-3, Kd = 0)
%     적분기   dI/dt = di,                  di 는 방식에 따라     (§3-6)
...
%
%  Kd = 0 으로 둔다. Kd 가 0 이 아니면 두 경로는 §3-4 의 c_d 때문에 애초에
%  다른 제어기이고, 그 차이는 안티와인드업과 아무 상관이 없다.
%  Kd is held at zero: with Kd non-zero the two paths are different controllers
%  by the setpoint weight of §3-4, and that difference has nothing to do with
%  anti-windup.
%
...
%  강의 §3-6 의 표가 이 네 줄을 그대로 싣는다.
%  Four halvings: two would not distinguish a fall from a coincidence, and the
%  table in §3-6 reproduces these four rows.
HS = [V.h, V.h/2, V.h/4, V.h/8];
dc = zeros(size(HS));
for j = 1:numel(HS)
    Vh = V;  Vh.h = HS(j);
    Rc = run_sim('W03_surge_control', Vh, 'aw_mode', 1, 'Kd', 0, 'pid_mode', 0);
    Rb = run_sim('W03_surge_control', Vh, 'aw_mode', 2, 'Kd', 0, 'pid_mode', 0);
...
R0   = run_sim('W03_surge_control', V, 'Ki', 0, 'aw_mode', 2);
...
%  §3-6 은 역계산이 두 가지 모양으로 그려진다고 말한다.
...
%
%  §3-6 claims that back-calculation is drawn two ways, with the gain outside
...
%
%  'outside' 와 'inside' 가 같은 값을 내야 §3-6 의 주장이 참이다.
%  If 'outside' and 'inside' agree, the claim of §3-6 holds.
t = (0:F.h:F.T).';
y = zeros(numel(t),1);
Y = 0;  I = 0;
for k = 1:numel(t)
...
in = Simulink.SimulationInput('W03_surge_control');
V.pid_mode = 1;   V.Kd = 0;
fn = fieldnames(V);
for i = 1:numel(fn), in = in.setVariable(fn{i}, V.(fn{i})); end
in = in.setBlockParameter('W03_surge_control/Surge controller/PID block', ...
...
%LOCAL_REPLAY  W03 의 속도 고리를 Simulink 없이 다시 구현한다.
%              The Week 3 speed loop, reimplemented without Simulink.
%
...
%  제어기 / the controller (§3-3, Kd = 0 이므로 미분항은 없다)
e     = ref(t,V) - x(1);
X_cmd = V.Kp*e + I;

%  배분 / the allocation (§3-5)
T = X_cmd/2;
