function ok = verify_w02_derivative()
%VERIFY_W02_DERIVATIVE  §2-4 의 미분항 부호 주장을 수치로 확인한다.
%                       Check, numerically, every claim §2-4 makes about the
%                       sign in front of the derivative term.
%
%      ok = verify_w02_derivative
%
%  ── 왜 이 파일이 있는가 / why this file exists ──────────────────────────
%  2026-09-16 검토 전까지 §2-4 는 "미분을 측정값에서 받는 형태의 마이너스는
%  교과서 PID 의 플러스를 다시 쓴 것일 뿐" 이라고 적고 있었다. 그 문장은
%  설정값이 계단처럼 구간마다 일정할 때만 참이다. u_d 가 시간에 따라 변하면
%  두 식은 서로 다른 제어기다.
%
%  Until the 2026-09-16 review §2-4 claimed that the minus sign of the
...
V = W02_vars;   % 게인을 여기서 다시 적지 않는다 / the gains have one home
...
KDS = [2 20 60 150];                      % §2-4·§E·§I 가 실제로 쓰는 값들
ok  = true;

fprintf('\n  W02 §2-4 — 미분항 부호 검증 / the sign of the derivative term\n');
...
fprintf('\n');
if ok
    fprintf('  ALL CHECKS PASSED — §2-4 의 주장이 수치와 맞는다.\n\n');
else
    fprintf('  ***** 불일치 있음. §2-4 를 다시 본다. *****\n\n');
end
end
