...
%   d - x_e = 4.50 m < R = 5,  |P - wp| = 10.97 m > R
...
\def\LEG{78}       % 다리 52 m
\def\RR{7.5}       % R = 5 m
\def\YV{15}        % y_e = 10 m  (= 2R)
\def\XV{71.25}     % d - x_e = 4.5 m  ->  x = 52 - 4.5 = 47.5 m
...
\begin{scope}
  ...
  \node[pathc, font=\scriptsize, anchor=north] at (0,-2)    {$\mathbf{wp}_k$};
  \node[pathc, font=\scriptsize, anchor=south] at (\LEG,2)  {$\mathbf{wp}_{k+1}$};

  \draw[errc, line width=0.8pt] (\XV,0) -- (\XV,-\YV);
  \node[errc, font=\scriptsize, anchor=west] at ({\XV+1.5},{-0.6*\YV}) {$y_e = 2R$};
...
\begin{scope}[shift={(96,0)}]
  ...
  \node[warn, font=\scriptsize, anchor=north west] at ({\XV+6},{-\YV+5})
       {$\lVert\mathbf{p}-\mathbf{wp}_{k+1}\rVert = 10.97$\,m};
...
\node[anchor=north west, align=left, text width=172mm, font=\scriptsize] at (0,-48) {%
  On the path the two agree exactly, because $y_e=0$ makes
  $\lVert\mathbf{p}-\mathbf{wp}_{k+1}\rVert = d-x_e$. Off it they do not:
  $\lVert\mathbf{p}-\mathbf{wp}_{k+1}\rVert = \sqrt{(d-x_e)^2+y_e^2} \ge d-x_e$,
  so \textbf{the circle test is always the stricter of the two}, by an amount that
  grows with $y_e$. Section F evaluates both on one position: the along-track
