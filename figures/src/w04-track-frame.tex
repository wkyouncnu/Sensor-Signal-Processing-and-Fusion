...
\def\XE{51.2}      % x_e = 32 m at 1.6 mm/m
\def\YE{22.4}      % y_e = 14 m
\def\LG{104}       % 다리 65 m
...
\begin{scope}[shift={(-4,54)}]
  ...
  \node[font=\small, anchor=east]  at (-0.8,9) {$N$};
  \node[font=\small, anchor=north] at (9,-0.8) {$E$};
...
\begin{scope}[rotate=28]
  ...
  \node[pathc, font=\small, anchor=north east] at (-1.5,-1.5) {$\mathbf{wp}_k$};
  \node[pathc, font=\small, anchor=north west] at ({\LG-16},-2) {$\mathbf{wp}_{k+1}$};
  \node[alng, font=\small, anchor=south]  at ({0.5*\XE},1.5) {$x_e$};
  \node[errc, font=\small, anchor=west]   at ({\XE+2},{-0.55*\YE}) {$y_e$};
...
\begin{scope}[shift={(122,0)}]
  ...
  \node[anchor=north west, font=\small] at (0,41)
       {$\pi_p = \mathrm{atan2}(E_{k+1}-E_k,\ N_{k+1}-N_k)$};
  ...
  \node[anchor=north west, font=\small] at (0,27)
       {$\begin{bmatrix} x_e \\ y_e \end{bmatrix}
         = \mathbf{R}(\pi_p)^{\!\top}
           \begin{bmatrix} N-N_k \\ E-E_k \end{bmatrix}$};

  \node[anchor=north west, align=left, text width=68mm, font=\scriptsize] at (0,6) {%
    $x_e$ — how far \textbf{along} the leg the vessel has come.
    The switching test of §4-6 uses it.\\[3pt]
    $y_e$ — how far \textbf{to the side}. The guidance law drives this one to
    zero.\\[6pt]
    Both are projections of the same vector onto the same orthonormal pair, so
    $x_e^2 + y_e^2 = \lVert\mathbf{p}-\mathbf{wp}_k\rVert^2$ — a rotation
