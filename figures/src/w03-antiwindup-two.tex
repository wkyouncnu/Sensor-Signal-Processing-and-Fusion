% figures/w03-antiwindup-two.svg — 두 안티와인드업이 적분기 입력에서 갈라진다.
%
%   bash _tools/tikz2svg.sh figures/src/w03-antiwindup-two.tex figures/w03-antiwindup-two.svg
%
% 두 방식은 루프의 나머지가 전부 같다. 다른 것은 **적분기 앞에서 무엇이 일어나는가**
% 하나뿐이므로, 그 자리만 나란히 놓고 나머지는 같은 모양으로 그린다.
% 전체 루프(포화 뒤가 왜 끊기는가)는 w03-windup.svg 가 이미 그렸다 — 여기서
...
\begin{scope}[shift={(104,0)}]
  ...
  \node[note, anchor=west, text width=76mm] at (-10,10)
       {feed the excess \emph{back} to steer the integrator};
  ...
  \node[anote, anchor=north, align=center, text width=54mm] at (26,-26)
       {$\dot I = K_i e - K_{\text{aw}}\varepsilon$, a continuous pull\\
        that settles with $\varepsilon = (K_i/K_{\text{aw}})\,e$, not zero};
\end{scope}
