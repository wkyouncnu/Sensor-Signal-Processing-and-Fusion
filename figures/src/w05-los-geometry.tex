% figures/w05-los-geometry.svg — LOS 유도의 기하. Fossen 의 표준 그림 배치.
%
%   matlab -batch "addpath _tools; w05_losgeo"          <- 먼저 기하를 계산하고
%   bash _tools/tikz2svg.sh figures/src/w05-los-geometry.tex figures/w05-los-geometry.svg
%
...
%
% 좌표는 전부 _tools/w05_losgeo.m 이 계산한다. **이 파일에는 산술이 없다** —
...
\node[font=\scriptsize, anchor=south] at (2,109.5) {$x^n$~(North)};
\draw[axis, -{Stealth[length=4pt,width=3pt]}] (2,96) -- (15,96);
\node[font=\scriptsize, anchor=west] at (15.5,96) {$y^n$~(East)};
...
\node[anchor=north west, align=left, text width=68mm, font=\scriptsize] at (-6,-84) {%
  ...
  Nothing else appears, which is exactly why §5-7 can say what the law cannot
  do.\\[4pt]
...
\node[anchor=north west, align=left, text width=68mm, font=\scriptsize] at (76,-84) {%
  ...
  The hull is at $\psi = \ValPsi^\circ$ with a crab angle $\beta = \ValBeta^\circ$, so
  ...
  $\chi_d$. This course carries a heading autopilot from Week~3, so §5-4
  commands $\psi_d$: the same expression with $\psi_d$ in place of $\chi_d$.
  The two agree exactly when $\beta = 0$, and §5-7 measures what the
