...
> [!important] 쉽게 쓰되, **중언부언하지 않는다**
> 쉬운 것과 반복하는 것은 다르다. 같은 말을 두 번 쓰면 강조가 아니라 소음이고,
> 두 번째를 읽는 사람은 첫 번째를 못 믿게 된다.
> **한 문단을 지웠을 때 잃는 것이 없으면 지운다.** 표를 산문으로 다시 읽어 주지
> 않고, 그림 설명에 그림에 적힌 글자를 옮기지 않으며, 바로 위 문단을 요약하는
> 콜아웃을 두지 않는다. 뒤에서 다시 필요하면 **§번호로 가리킨다.**
> 다만 **줄이지 않는 것 셋** — 유도의 중간 단계, 버린 항의 근거, 측정 수치의 출처.
> 이것들은 반복이 아니라 내용이다 → `standing-orders.md` §9-5

> [!important] 문서와 코드가 맞는지 **기계로** 본다
> 문서가 부르는 스크립트가 없으면 학생은 첫 줄에서 막히고, 그 다음부터 문서를
> 믿지 않는다. `vault_check.sh` **§15** 가 주차 문서의 `WXX_…` 토큰을 전부 모아
> `.m` · `.slx` · `img/*.png` · 폴더 중 무엇으로든 실재하는지 본다
